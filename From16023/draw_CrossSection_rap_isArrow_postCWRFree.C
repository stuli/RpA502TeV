...
void draw_CrossSection_rap_isArrow_postCWRFree(int ppAA=1, bool isArrow=false) //1=pp, 2=AA
{
  setTDRStyle();
...
//  double relsys = 0.1;
  ...
  int npoint[nState];
  for (int is=0; is<nState; is++){
    fInSys[is] = new TFile(Form("../Systematic/postCWR_mergedSys_free_ups%ds_asymHi.root",is+1),"READ");
...
//  globtex->DrawLatex(0.27, sz_init-sz_shift-sz_step*2, "|#eta^{#mu}| < 2.4");
  ...
  CMS_lumi( c1, ppAA, iPos );
	...
	c1->Update();
  c1->SaveAs(Form("CrossSection_vs_rap_%s_postCWRFree.pdf",sz_ppAA.Data()));
  c1->SaveAs(Form("CrossSection_vs_rap_%s_postCWRFree.png",sz_ppAA.Data()));

	///////////////////////////////////////////////////////////////////
	//// save as a root file
	TFile *outFile = new TFile("CrossSection_vs_rap_postCWRFree.root", "RECREATE");
	outFile->cd();
	for (int is=0; is<nState; is++){
