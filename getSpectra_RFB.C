...
void getSpectra_RFB(int state = 1 ) {  
  ...
  TH1D* hptSigPAF = new TH1D("hHFPAF",";E_{T}^{HF|#eta|>4} (GeV);",nHFBins,HFBin);
  TH1D* hptSigPAB = new TH1D("hHFPAB",";E_{T}^{HF|#eta|>4} (GeV);",nHFBins,HFBin);
  ...
  for ( int ipt = 1 ; ipt<=nHFBins ; ipt++) {
    valErr yieldPP = getYield(state, kPADATA, 0,30, -1.93, 0.00, 0,200, HFBin[ipt-1],HFBin[ipt],0,400);
    valErr yieldPA = getYield(state, kPADATA, 0,30,  0.00, 1.93, 0,200, HFBin[ipt-1],HFBin[ipt],0,400);
  ...
  hRFB_HF = (TH1D*)hptSigPAF->Clone("rfb_hf");
  ...
  gRFB_HF->SetName("gRFB_HF");
  gRFB_HF->SetTitle("gRFB_vs_HF");
  gRFB_HF->Write();
  wf->Close();
}
