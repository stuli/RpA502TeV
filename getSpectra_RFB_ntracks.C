...
void getSpectra_RFB_ntracks(int state = 1 ) {  
  ...
  TH1D* hptSigPP = new TH1D("hptPP",";N_{tracks};",nNtracksBins,NtracksBin);
  TH1D* hptSigPA = new TH1D("hptPA",";N_{tracks};",nNtracksBins,NtracksBin);
  ...
  for ( int ipt = 1 ; ipt<=nNtracksBins ; ipt++) {
    valErr yieldPP;
    if(state==3) yieldPP = getYield(state, kPADATA, 0,30, -rap_rfbmax, rap_rfbmin,0,200,0.00,400.00,(int)NtracksBin[ipt-1],(int)NtracksBin[ipt]);
    else yieldPP = getYield(state, kPADATA, 0,30, -rap_rfbmax, -rap_rfbmin,0,200,0.00,400.00,(int)NtracksBin[ipt-1],(int)NtracksBin[ipt]);
    valErr yieldPA = getYield(state, kPADATA, 0,30,  rap_rfbmin,  rap_rfbmax,0,200,0.00,400.00,(int)NtracksBin[ipt-1],(int)NtracksBin[ipt]);
