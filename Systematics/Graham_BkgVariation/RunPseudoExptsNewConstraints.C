...
#include "FitDataWithConstraintsAltBkgNewConstraints.C"
...
void RunPseudoExptsConstraintsNomSeed(
		int collId = 1,
			...
			)
{
	...
	if (collId == kPADATA)
		...
		collLabel = "PP";
	TFile* outfile = new TFile(Form("ResultsBkgConstrained/PseudoExpResults_%s_pt%.1f-%.1f_y%.2f-%.2f.root",collLabel,ptLow,ptHigh,yLow,yHigh),"RECREATE");
	...
	for (int i = 0; i < numTrials; i++)
	{
		...
		//Alternate fit
		cout << "DOING ALTERNATE FIT TO PSEUDO" << endl;
		FitDataWithConstraintsAltBkgNewConstraints(collId,ptLow,ptHigh,yLow,yHigh,cLow,cHigh,muPtCut,whichModel,resultVector,pseudoData,NomFileName);
		...
		//Nominal fit
		cout << "DOING NOMINAL FIT TO PSEUDO" << endl;
		FitDataWithConstraintsAltBkgNewConstraints(collId,ptLow,ptHigh,yLow,yHigh,cLow,cHigh,muPtCut,0,resultVector,pseudoData,NomFileName);
