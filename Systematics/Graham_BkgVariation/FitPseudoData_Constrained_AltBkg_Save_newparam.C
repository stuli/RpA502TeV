//Generates pseudodata from the nominal fit in the corresponding kinematic bin and fits twice with nominal and alternative background PDFs, then repeats for multiple trials and saves the resulting yields from all fits.
//Also saves chi^2/ndf of all fits and a small sample of fitting plots.
...
void FitPseudoData_Constrained_AltBkg_Save_newparam( 
       int collId = kPPDATA,  
       float ptLow=0, float ptHigh=6, 
       float yLow=1.2, float yHigh=1.93,
			...
			)
{
  ...
  RooRandom::randomGenerator()->SetSeed(startTime.GetSec()); //Set random seed
  ...
  //Location of nominal fit directory
  TString nomDirectory = "../../../JaredNomFitsConstrained";
  //Location to output results
  TString outDirectory = "PseudoResults_newparam";
  ...
  TString NomFileName = Form("%s/nomfitresults_upsilon_%s.root",nomDirectory.Data(),kineLabel.Data());
  ...
  TString outFileName = Form("%s/PseudoExpResults_%s.root",outDirectory.Data(),kineLabel.Data());
  ...
  for (int itrial = 0; itrial<numtrials; itrial++) {
    ...
    // Set up crystal ball shapes
      ...
      //BACKGROUND
      RooAbsPdf* bkg;
      RooAddPdf* modelNoC; //Need to declare up here because low pt alt bkg builds it differently
      ...
      if (thisModel==1)
      {
        ...
        if (collId == kPADATA)
          lowPtBkgFile = new TFile("altBkgModels_OS_boostedAll.root","READ");
        else if (collId == kPPDATA)
          lowPtBkgFile = new TFile("altBkgModelsPP.root","READ");
        bkgWs = (RooWorkspace*)lowPtBkgFile->Get("altBkgWorkspace");
        for (int i = 1; i <= 4; i++)
        {
          bkgSumExpErf[i] = bkgWs->pdf(Form("bkgSumExpErf_%d",i));
        }
        modelNoC = new RooAddPdf("modelNoC","1S+2S+3S + Bkg",RooArgList(*cb1s, *cb2s, *cb3s, *bkgSumExpErf[1],*bkgSumExpErf[2],*bkgSumExpErf[3],*bkgSumExpErf[4]),RooArgList(*nSig1s,*nSig2s,*nSig3s,a1,a2,a3,a4));
      }
      else if (thisModel==3)
      {
        ...
        bkg = bkgPow;
        modelNoC = new RooAddPdf("modelNoC","1S+2S+3S + Bkg",RooArgList(*cb1s, *cb2s, *cb3s, *bkg),RooArgList(*nSig1s,*nSig2s,*nSig3s,*nBkg));
      }
      else if (thisModel==0)
      {
      ...
      {
        if (imodel != 0)
          cfit->SaveAs(Form("%s/plots/altfitresults_upsilon_%s_t%d.png",outDirectory.Data(),kineLabel.Data(),itrial));
        else
          cfit->SaveAs(Form("%s/plots/nomfitresults_upsilon_%s_t%d.png",outDirectory.Data(),kineLabel.Data(),itrial));
      }
    ...
    if (chisqtest[0]>maxchisq || chisqtest[1]>maxchisq) {
      itrial--;
  ...
  TTimeStamp endTime;
  double elapsedTime = 0.000000001*(endTime.GetNanoSec()-startTime.GetNanoSec()) + (endTime.GetSec()-startTime.GetSec());
  double elapsedHours = elapsedTime / 3600;
  cout << Form("Elapsed time: %.0f sec (%.1f hr)",elapsedTime,elapsedHours) << endl;
}
