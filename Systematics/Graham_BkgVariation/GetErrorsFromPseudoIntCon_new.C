...
void GetErrorsFromPseudoIntCon_new(int saveresults = 0)
{
	int whichState = 0;
	int firstBin = 0;
	int lastBin = -1;
	...
	for (int diffBin = 0; diffBin < 3; diffBin++) //0=standard bins, 1=ptlow,ymi, 2=pthigh,ypl
	{
		...
		for (int iState = whichState; iState <= maxState; iState++)
		{
			for (int i = firstBin; i <= lastBin+1; i++)
			{
				...
				TString inFileNamePA = Form("PseudoResults/PseudoExpResults_PA_DATA_pt%.1f-%.1f_y%.2f-%.2f_muPt4.0.root",ptLow,ptHigh,yLow,yHigh);
				...
				TString inFileNamePP = Form("PseudoResults/PseudoExpResults_PP_DATA_pt%.1f-%.1f_y%.2f-%.2f_muPt4.0.root",ptLow,ptHigh,yLowPP,yHighPP);
				...
				TNtuple* tupPA = (TNtuple*)(inFilePA->Get("ntuple"));
				TNtuple* tupPP = (TNtuple*)(inFilePP->Get("ntuple"));
				...
				c1->SaveAs(Form("PseudoResults/plots/pdfs/") + outFileName + ".pdf");
				c1->SaveAs(Form("PseudoResults/plots/pngs/") + outFileName + ".png");
	...
	*/
	...
	for (int i=0; i<8; i++)
	{
		...
		for (int j=0; j<10; j++)
		{
			...
			/*OLD
			TString inFileName = Form("PseudoResults/PseudoExpResults_PA_DATA_pt%.1f-%.1f_y%.2f-%.2f_hfsum%.2f-%.2f_ntracks%d-%d.root",ptLow,ptHigh,yLow,yHigh,hfLow,hfHigh,ntLow,ntHigh);
			...
			TString inFileNamePL = Form("PseudoResults/PseudoExpResults_PA_DATA_pt%.1f-%.1f_y%.2f-%.2f_muPt4.0_hfsum%.2f-%.2f_ntracks%d-%d.root",ptLow,ptHigh,yLow,yHigh,hfLow,hfHigh,ntLow,ntHigh);
			TFile* inFilePL = new TFile(inFileNamePL,"READ");
			if (inFilePL->IsZombie())
			{
				cout << "LINE " << lineNumber << " HAS NO PL FILE" << endl;
				lineValues[lineNumber] = "& * & * & *";
				continue;
			}
			...
			float yLowMI = -yHigh;
			float yHighMI = -yLow;
			if (TMath::Abs(yHighMI) < 0.01) yHighMI=0;
			TString inFileNameMI = Form("PseudoResults/PseudoExpResults_PA_DATA_pt%.1f-%.1f_y%.2f-%.2f_muPt4.0_hfsum%.2f-%.2f_ntracks%d-%d.root",ptLow,ptHigh,yLowMI,yHighMI,hfLow,hfHigh,ntLow,ntHigh);
			TFile* inFileMI = new TFile(inFileNameMI,"READ");
			if (inFileMI->IsZombie())
			{
				cout << "LINE " << lineNumber << " HAS NO MI FILE" << endl;
				lineValues[lineNumber] = "& * & * & *";
				continue;
			}
			
			TH1F* histoPL = new TH1F("histoPA",Form("%dS %%Diff in Yields for PA",iState),100,0,20);
			TH1F* histoMI = new TH1F("histoPP",Form("%dS %%Diff in Yields for PP",iState),100,0,40);
			TH1F* histoRfb = new TH1F("histoRpA",Form("%dS %%Diff in Yields for R_{pA}",iState),100,0,100);
			histoPL->SetXTitle("%Diff");
			histoMI->SetXTitle("%Diff");
			histoRfb->SetXTitle("%Diff");
			
			TNtuple* tupPL = (TNtuple*)(inFilePL->Get("ntuple"));
			TNtuple* tupMI = (TNtuple*)(inFileMI->Get("ntuple"));
			...
			hThisPL->SetBinContent(outbin,PLdiff);
			hThisMI->SetBinContent(outbin,MIdiff);
			hThisRfb->SetBinContent(outbin,Rfbdiff);
	...
	cout << "*****DOING INTEGRATED RFB*****" << endl;
	...
	for (int iState = 1; iState <= 2; iState++)
	{
		...
		TFile* inFilePL = new TFile("PseudoResults/PseudoExpResults_PA_DATA_pt0.0-30.0_y0.00-1.93_muPt4.0.root","READ");
		if (inFilePL->IsZombie())
		{
			cout << "INTEGRATED RFB HAS NO PL FILE" << endl;
			continue;
		}
		TFile* inFileMI = new TFile("PseudoResults/PseudoExpResults_PA_DATA_pt0.0-30.0_y-1.93-0.00_muPt4.0.root","READ");
		if (inFileMI->IsZombie())
		{
...
void calcError(TNtuple* tup1, double* results, int state, TH1F* h1)
{
	...
	tup1->SetBranchAddress(Form("yield%dsAlt",state),&sigAlt1);
	tup1->SetBranchAddress(Form("yield%dsNom",state),&sigNom1);
...
void calcError(TNtuple* tup1, TNtuple* tup2, double* results, int state, TH1F* h1, TH1F* h2, TH1F* hR)
{
	...
	tup2->SetBranchAddress(Form("yield%dsAlt",state),&sigAlt2);
	tup2->SetBranchAddress(Form("yield%dsNom",state),&sigNom2);
