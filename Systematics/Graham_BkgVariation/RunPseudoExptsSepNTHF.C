			...
			)
{
	...
	if (collId == kPADATA)
		...
		collLabel = "PP";
	TFile* outfile = new TFile(Form("ResultsBkg/PseudoExpResults_%s_pt%.1f-%.1f_y%.2f-%.2f_hfsum%.2f-%.2f_ntracks%d-%d.root",collLabel,ptLow,ptHigh,yLow,yHigh,hfsumLow,hfsumHigh,ntrackLow,ntrackHigh),"RECREATE");
