...
#include "FitDataWithConstraintsAltBkgNomSeed.C"
...
void RunPseudoExptsConstraintsNomSeedNTHF(
		int collId = 1,
		float ptLow=0, float ptHigh=30, 
		float yLow=0.0, float yHigh=1.93,//Run 1 has p going in -z direction
		int ntrackLow = 0, int ntrackHigh = 400,
		float hfsumLow = 0, float hfsumHigh = 120,
			...
			)
{
	...
	if (collId == kPADATA)
		...
		collLabel = "PP";
	TFile* outfile = new TFile(Form("ResultsBkgConstrained/PseudoExpResults_%s_pt%.1f-%.1f_y%.2f-%.2f_hfsum%.2f-%.2f_ntracks%d-%d.root",collLabel,ptLow,ptHigh,yLow,yHigh,hfsumLow,hfsumHigh,ntrackLow,ntrackHigh),"RECREATE");
	...
	for (int i = 0; i < numTrials; i++)
	{
		...
		//Alternate fit
		FitDataWithConstraintsAltBkgNomSeed(collId,ptLow,ptHigh,yLow,yHigh,cLow,cHigh,muPtCut,whichModel,resultVector,pseudoData,NomFileName);
		...
		//Nominal fit
		FitDataWithConstraintsAltBkgNomSeed(collId,ptLow,ptHigh,yLow,yHigh,cLow,cHigh,muPtCut,0,resultVector,pseudoData,NomFileName);
