...
void doFitUpsilon_BkgSameSign_4th(
       int collId = kAADATA,  
       ...
       bool fixParameters=0 )
{
  ...
  else if ( collId == kAADATA) f1 = new TFile("/home/deathold/work/CMS/analysis/Upsilon_RAA/upsilonRAA5TeV/skimmedFiles/yskimPbPb_L1DoubleMu0PD_Trig-L1DoubleMu0_SSign_EP-OppositeHF_201612202053_.root");
...
//  cout << " *** NLL : " << theNLL << endl;
  ...
  float pos_text_x = 0.33;
  float pos_text_y = 0.513;
...
//  drawText(Form("Signal Function : %s CB", SignalCB.Data() ), 0.55,0.54,1,14);
  ...
  TLegend* fitleg = new TLegend(0.76,0.55,0.91,0.7); fitleg->SetTextSize(19);
...
//  fitleg->AddEntry(myPlot2->findObject("Sig1S"),"Signal","l");
  ...
  RooHist* hpull = myPlot2->pullHist("dataHist","bkgPDF");
...
*/
  ...
  TFile* outf = new TFile(Form("BkgFit_4th_SameSign_fitresults_upsilon_DoubleCB_%s.root",kineLabel.Data()),"recreate");
  outh->Write();
  c1->SaveAs(Form("BkgFit_4th_SameSign_fitresults_upsilon_DoubleCB_%s.pdf",kineLabel.Data()));
  c1->SaveAs(Form("BkgFit_4th_SameSign_fitresults_upsilon_DoubleCB_%s.png",kineLabel.Data()));
  c1->Write();
  ws->Write();
