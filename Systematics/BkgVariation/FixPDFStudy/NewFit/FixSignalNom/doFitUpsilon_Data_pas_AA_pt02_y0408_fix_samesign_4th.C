       ...
       bool fixParameters=0  )
{
  ...
  RooRealVar *nSig2s= new RooRealVar("nSig2s"," 2S signals",-10,3600);
  RooRealVar *nSig3s= new RooRealVar("nSig3s"," 3S signals",-10,2600);
  ...
  TFile* outf = new TFile(Form("Data_NomSigFit_4th_fitresults_upsilon_DoubleCB_%s.root",kineLabel.Data()),"recreate");
  outh->Write();
  c1->SaveAs(Form("Data_NomSigFit_4th_fitresults_upsilon_DoubleCB_%s.pdf",kineLabel.Data()));
  c1->SaveAs(Form("Data_NomSigFit_4th_fitresults_upsilon_DoubleCB_%s.png",kineLabel.Data()));
  c1->Write();
  ws->Write();
