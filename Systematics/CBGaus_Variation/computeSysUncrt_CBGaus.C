...
void computeSysUncrt_CBGaus(int state=1)
{ 
  ...
  TFile *wf = new TFile(Form("sys_bkgPDFVariaion_%ds.root",state),"recreate");
...
double getRMS(TString fileName , int state) 
{
  ...
  { 
    ...
    else fin = Form("MassPlot/%s.root", fileName.Data());
...
double getFinalUnc(double pp, double aa)
{
  ...
  FinalUnc = TMath::Sqrt(aa*aa+pp*pp);
  return FinalUnc;
}
