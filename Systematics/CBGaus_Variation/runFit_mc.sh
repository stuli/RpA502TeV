...
outputDir='fitResults'

#outputDir='Test/fixParam'${fixParam}'MuPt'${muPtCut}'_fullRap_mar01_bkgtest'${fbkg_ch}''
mkdir -p $outputDir
cp doFitUpsilon_GausCB_MC.C *.h $outputDir

root -l -b <<EOF
.L doFitUpsilon_GausCB_MC.C++
.q
EOF
...
do
    ...
    do
	#for rap in '1.2,2.4' '0,2.4' '0,0.4' '0.4,0.8' '0.8,1.2' '1.2,1.6' '1.6,2.0' '2.0,2.4'
	for rap in  '0,2.4' 
	#for rap in '0,1.2'
  #for rap in '0,2.4'
	do
	    echo  $outputDir 'doFitUpsilon_GausCB_MC.C('$i','$pt','$rap',0,200,'$muPtCut')'
	    root -l -b -q $outputDir 'doFitUpsilon_GausCB_MC.C('$i','$pt','$rap',0,200,'$muPtCut')'
	    #./condor_root.sh $outputDir 'doFitUpsilon_GausCB_MC.C('$i','$pt','$rap',0,200,'$muPtCut')'
	done
    done
done
