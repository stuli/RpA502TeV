...
{            
  ...
  else if (iPeriod==1051)
    {
      lumiText += lumi_PbPb2015_1;
      //lumiText += label;
      lumiText2 += " #sqrt{s_{NN}} = 5.02 TeV";
    }
  else if (iPeriod==106)
    {
      lumiText += lumi_pPb2016;
      lumiText2 += " #sqrt{s_{NN}} = 8.16 TeV";
    }
  ...
  if( !outOfFrame )
      ...
      if( drawLogo )
	{
	  ...
	  TPad* pad_logo = new TPad("logo","logo", xl_0*1.1, yl_0*1.1, xl_1, yl_1 );
	  pad_logo->Draw();
	  pad_logo->cd();
	  CMS_logo->Draw("X");
	  pad_logo->Modified();
	  ...
	  if( writeExtraText ) 
	    {
	      ...
	      latex.DrawLatex(posX_, posY_- relExtraDY*cmsTextSize*t*0.8, extraText);
	    }
	}
    }
