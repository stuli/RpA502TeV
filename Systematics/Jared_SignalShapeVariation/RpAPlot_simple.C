			...
			) 
{
  ...
  TString PAFileName = Form("nomfitresults_upsilon_%s.root",PAkineLabel.Data());
  cout << PAFileName << endl;
  TFile* PAFile = TFile::Open(PAFileName,"READ");
  RooWorkspace *PAws = (RooWorkspace*)PAFile->Get("workspace");

  TString PPkineLabel = getKineLabel (collId, ptLow, ptHigh, 0.0, yHigh, muPtCut, cLow, cHigh, dphiEp2Low, dphiEp2High);
  TString PPFileName = Form("nomfitresults_upsilon_%s.root",PPkineLabel.Data());
  cout << PPFileName << endl;
  TFile* PPFile = TFile::Open(PPFileName,"READ");
  RooWorkspace *PPws = (RooWorkspace*)PPFile->Get("workspace");

  RooAbsPdf* PAmodel = PAws->pdf("model");
  PAmodel->SetName("PAmodel");
  RooWorkspace *ws = new RooWorkspace("workspace");
  ws->import(*PAmodel);

  //RooAbsPdf* PPmodel = PPws->pdf("model");
  //PPmodel->SetName("PPmodel");
  //ws->import(*PPmodel);

  //import PA dataset
  RooAbsData* reducedDS = PAws->data("reducedDS");
  ...
  ws->pdf("PAmodel")->plotOn(myPlot2,Name("modelHist"));
  //ws->pdf("PAmodel")->plotOn(myPlot2,Name("Sig1S"),Components(RooArgSet(*cb1s)),LineColor(kOrange+7),LineWidth(2),LineStyle(2));
  //ws->pdf("PAmodel")->plotOn(myPlot2,Components(RooArgSet(*cb2s)),LineColor(kOrange+7),LineWidth(2),LineStyle(2));
  //ws->pdf("PAmodel")->plotOn(myPlot2,Components(RooArgSet(*cb3s)),LineColor(kOrange+7),LineWidth(2),LineStyle(2));
  //ws->pdf("PAmodel")->plotOn(myPlot2,Name("bkgPDF"),Components(RooArgSet(*bkg)),LineColor(kBlue),LineStyle(kDashed),LineWidth(2));
