...
double VaryICsFitter( 
       int collId = kPADATA,  
       float ptLow=0, float ptHigh=30, 
       float yLow=-1.93, float yHigh=1.93,//Run 1 has p going in -z direction
       int cLow=0, int cHigh=300,
       float muPtCut=4.0,
       bool whichModel=0,   // Nominal = 0. Alternative = 1.
       double* paramsICsptr=0,
       double* paramsptr=0
			) 
{

  double sigma1s_1_init = *paramsICsptr;
  double x1s_init = *(paramsICsptr+1);
  double alpha1s_1_init = *(paramsICsptr+2);
  double n1s_1_init = *(paramsICsptr+3);
  double f1s_init = *(paramsICsptr+4);
  double err_sigma_init = *(paramsICsptr+5);
  double err_mu_init = *(paramsICsptr+6);
  double m_lambda_init = *(paramsICsptr+7);

  /*double sigma1s_1_init = 0.0562682;
  double x1s_init = 4.82618;
  double alpha1s_1_init = 1.7908;
  double n1s_1_init = 2.0896;
  double f1s_init = 0.376791;
  double err_sigma_init = 18.9085;
  double err_mu_init = 13.3842;
  double m_lambda_init = 4.97098;*/
  ...
  RooRealVar n1s_1("n1s_1","power order", n1s_1_init , 1.0, 5.0);
...
if (whichModel) {
  ...
  //Fix Parameters
  //sigma1s_1.setVal(0.07);
  //sigma1s_1.setConstant(kTRUE);
  //n1s_1.setVal(2.96726);
  //n1s_1.setConstant(kTRUE);
  //alpha1s_1.setVal(1.61832);
  //alpha1s_1.setConstant(kTRUE);
  //err_mu.setVal(8.39745);
  //err_mu.setConstant(kTRUE);
  ...
  *paramsptr = ws->var("sigma1s_1")->getVal();
  *(paramsptr+1) = ws->var("x1s")->getVal();
  *(paramsptr+2) = ws->var("alpha1s_1")->getVal();
  *(paramsptr+3) = ws->var("n1s_1")->getVal();
  *(paramsptr+4) = ws->var("f1s")->getVal();
  if (ptLow<5) {
  *(paramsptr+5) = ws->var("#sigma")->getVal();
  *(paramsptr+6) = ws->var("#mu")->getVal();
  }
  else {
  *(paramsptr+5) = err_sigma_init;
  *(paramsptr+6) = err_mu_init;
  }
  *(paramsptr+7) = ws->var("#lambda")->getVal();
...
TString outFileName;
  ...
  return temp1err+temp2err+temp3err;
