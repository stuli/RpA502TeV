...
using namespace std;
void GetErrorEstimates_HFNtracksIntBin(int whichUpsilon=1, int binmode=0) {
//0=hfmode, 1=ntmode
  ...
  float hfbins[2] = {0,120};
  int ntracksbins[2] = {0,400};
  ...
  TString hfntracksbins = "_hfbinsIntBin.root";
  int numbins = numhfbins;
  if (binmode==1) {
    hfntracksbins = "_ntracksbinsIntBin.root";
    numbins = numntbins;
  }
  ...
  TNtuple* ntuple12 = new TNtuple("ntuple","Error estimates in y bins","binlowlim:binuplim:binlowlimy:binuplimy:pPb1sFerr:pPb1sBerr:RFBErr",8);
