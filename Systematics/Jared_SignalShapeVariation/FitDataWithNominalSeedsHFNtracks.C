...
double FitDataWithNominalSeedsHFNtracks( 
       int collId = kPADATA,  
			...
			) 
{
  ...
  TString directory = "/media/jared/Acer/Users/Jared/Desktop/Ubuntu_Overflow/Fits/FitsWithNominalSeeds/";
  //TString directory = "/media/jared/Acer/Users/Jared/Desktop/Ubuntu_Overflow/Fits/AlternateFitsWithNominalSeeds/";
  ...
  double paramsupper[8] = {0.2, 1.0, 5.0, 4.0, 1.0, 15.0, 15.0, 25.0};
...
if (whichModel) {
  ...
  TString MCFileName = Form("../../SignalFitting/MCFitsWithnfix/MC1Snomfitresults_upsilon_%s.root",kineLabel.Data());
  cout << MCFileName << endl;
  TFile* MCFile = TFile::Open(MCFileName,"READ");
  RooWorkspace *MCws = (RooWorkspace*)MCFile->Get("workspace");
  MCFile->Close("R");

  //fix parameters
  if (collId==kPADATA) {
    f1s->setVal(MCws->var("f1s")->getVal());
    f1s->setConstant();
  }
  n1s_1.setConstant();
  //x1s->setVal(0.596867);
  x1s->setConstant();
  //alpha1s_1.setVal(3.40328);
