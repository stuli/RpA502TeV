...
void SuperRpARAAPlot_SameBkg_withoutPull_justpp( 
       int collId = kPPDATA,  
...
if (whichModel) {
  ...
  ws->pdf("model")->plotOn(myPlot2,Name("modelHist"),NormRange("full"),LineWidth(2));
...
//*****************************************************************************
  ...
  TFile* AANomFile = TFile::Open(NomFileName,"READ");
...
if (whichModel) {
  ...
  double AAnSig1s_init = AANomws->var("nSig1s")->getVal()*RAA1;
  double AAnSig2s_init = AANomws->var("nSig2s")->getVal()*RAA2;
  double AAnSig3s_init = AANomws->var("nSig3s")->getVal()*RAA3;
...
//*****************************************************************************
  ...
  TFile* PANomFile = TFile::Open(NomFileName,"READ");
...
if (whichModel) {
  ...
  double PAnSig1s_init = PANomws->var("nSig1s")->getVal()*RpA1;
  double PAnSig2s_init = PANomws->var("nSig2s")->getVal()*RpA2;
  double PAnSig3s_init = PANomws->var("nSig3s")->getVal()*RpA3;
...
//*****************************************************************************
  ...
  float AANorm = (AANomws->var("nSig1s")->getVal()*RAA1) + (AANomws->var("nSig2s")->getVal()*RAA2) + (AANomws->var("nSig3s")->getVal()*RAA3) + (Nomws->var("nBkg")->getVal());
  float PANorm = (PANomws->var("nSig1s")->getVal()*RpA1) + (PANomws->var("nSig2s")->getVal()*RpA2) + (PANomws->var("nSig3s")->getVal()*RpA3) + (Nomws->var("nBkg")->getVal());
  PAws->pdf("PAmodel")->plotOn(RpAmyPlot2,Name("PAmodelHistRpA"),LineColor(kGreen+2),LineWidth(2),LineStyle(5),Normalization(PANorm,RooAbsReal::NumEvent),Range(8.8,10.8),NormRange("full"));
  AAws->pdf("AAmodel")->plotOn(RpAmyPlot2,Name("AAmodelHistRpA"),LineColor(kRed+1),LineWidth(2),LineStyle(2),Normalization(AANorm,RooAbsReal::NumEvent),Range(8.8,10.8),NormRange("full"));
  ...
  if (writeExtraText) {
    ...
    RpAc1->SaveAs("RpARAAVisualPlot_noPreliminary.png");
    RpAc1->SaveAs("RpARAAVisualPlot_noPreliminary.pdf");
  }
