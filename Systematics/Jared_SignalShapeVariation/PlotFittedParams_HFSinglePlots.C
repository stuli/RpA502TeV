...
void PlotFittedParams_HFSinglePlots(int whichUpsilon=1) {
  ...
  //declare histograms
  TH1F* halphapt = new TH1F("halphapt","alpha vs hf",numhfbins,hfbinsptr);
  TH1F* hf1spt = new TH1F("hf1spt","f1s vs hf",numhfbins,hfbinsptr);
  TH1F* hmasspt = new TH1F("hmasspt","mass vs hf",numhfbins,hfbinsptr);
  TH1F* hn1spt = new TH1F("hn1spt","n1s vs hf",numhfbins,hfbinsptr);
  TH1F* hsigma1spt = new TH1F("hsigma1spt","sigma vs hf",numhfbins,hfbinsptr);
  TH1F* hx1spt = new TH1F("hx1spt","x1s vs hf",numhfbins,hfbinsptr);
  TH1F* herrmupt = new TH1F("herrmupt","errmu vs hf",numhfbins,hfbinsptr);
  TH1F* herrsigmapt = new TH1F("herrsigmapt","errsigma vs hf",numhfbins,hfbinsptr);
  TH1F* hlambdapt = new TH1F("hlambdapt","errlambda vs hf",numhfbins,hfbinsptr);
  TH1F* hnSig1spt = new TH1F("hnSig1spt","nSig1s vs hf",numhfbins,hfbinsptr);
  TH1F* hnSig2spt = new TH1F("hnSig2spt","nSig2s vs hf",numhfbins,hfbinsptr);
  TH1F* hnSig3spt = new TH1F("hnSig3spt","nSig3s vs hf",numhfbins,hfbinsptr);
  TH1F* hnBkgpt = new TH1F("hnBkgpt","nBkg vs hf",numhfbins,hfbinsptr);

  TH1F* halphay = new TH1F("halphay","alpha vs hf",numhfbins,hfbinsptr);
  TH1F* hf1sy = new TH1F("hf1sy","f1s vs hf",numhfbins,hfbinsptr);
  TH1F* hmassy = new TH1F("hmassy","mass vs hf",numhfbins,hfbinsptr);
  TH1F* hn1sy = new TH1F("hn1sy","n1s vs hf",numhfbins,hfbinsptr);
  TH1F* hsigma1sy = new TH1F("hsigma1sy","sigma vs hf",numhfbins,hfbinsptr);
  TH1F* hx1sy = new TH1F("hx1sy","x1s vs hf",numhfbins,hfbinsptr);
  TH1F* herrmuy = new TH1F("herrmuy","errmu vs hf",numhfbins,hfbinsptr);
  TH1F* herrsigmay = new TH1F("herrsigmay","errsigma vs hf",numhfbins,hfbinsptr);
  TH1F* hlambday = new TH1F("hlambday","errlambda vs hf",numhfbins,hfbinsptr);
  TH1F* hnSig1sy = new TH1F("hnSig1sy","nSig1s vs hf",numhfbins,hfbinsptr);
  TH1F* hnSig2sy = new TH1F("hnSig2sy","nSig2s vs hf",numhfbins,hfbinsptr);
  TH1F* hnSig3sy = new TH1F("hnSig3sy","nSig3s vs hf",numhfbins,hfbinsptr);
  TH1F* hnBkgy = new TH1F("hnBkgy","nBkg vs hf",numhfbins,hfbinsptr);
  ...
  //1S backward loop
  for (int ipt = 0; ipt<numhfbins; ipt++) {

    hfLow = *(hfbinsptr+ipt);
    hfHigh = *(hfbinsptr+ipt+1);
  ...
  //draw pt plots
  calpha->cd();
  halphapt->SetXTitle("E_T^{HF}");
  halphapt->GetYaxis()->SetRangeUser(plotlimlower[2],plotlimupper[2]);
  halphapt->Draw();
  //halphapt->Fit("pol1");
  TLine *alphaptlineLow = new TLine(0,paramslower[2],120,paramslower[2]);
  alphaptlineLow->SetLineColor(kRed);
  alphaptlineLow->Draw();
  TLine *alphaptlineHigh = new TLine(0,paramsupper[2],120,paramsupper[2]);
  alphaptlineHigh->SetLineColor(kRed);
  alphaptlineHigh->Draw();

  cf1s->cd();
  hf1spt->SetXTitle("E_T^{HF}");
  hf1spt->GetYaxis()->SetRangeUser(plotlimlower[4],plotlimupper[4]);
  hf1spt->Draw();
  //hf1spt->Fit("pol1");
  TLine *f1sptlineLow = new TLine(0,paramslower[4],120,paramslower[4]);
  f1sptlineLow->SetLineColor(kRed);
  f1sptlineLow->Draw();
  TLine *f1sptlineHigh = new TLine(0,paramsupper[4],120,paramsupper[4]);
  f1sptlineHigh->SetLineColor(kRed);
  f1sptlineHigh->Draw();

  cmass->cd();
  hmasspt->SetXTitle("E_T^{HF}");
  hmasspt->GetYaxis()->SetRangeUser(9.35,9.6);
  hmasspt->Draw();
  //hmasspt->Fit("pol1");
  TLine *massptlinepdg = new TLine(0,9.46,120,9.46);
  massptlinepdg->SetLineColor(3);
  massptlinepdg->Draw();
  TLine *massptlineLow = new TLine(0,9.36,120,9.36);
  massptlineLow->SetLineColor(kRed);
  massptlineLow->Draw();
  TLine *massptlineHigh = new TLine(0,9.56,120,9.56);
  massptlineHigh->SetLineColor(kRed);
  massptlineHigh->Draw();

  cn1s->cd();
  hn1spt->SetXTitle("E_T^{HF}");
  hn1spt->GetYaxis()->SetRangeUser(plotlimlower[3],plotlimupper[3]);
  hn1spt->Draw();
  //hn1spt->Fit("pol1");
  TLine *n1sptlineLow = new TLine(0,paramslower[3],120,paramslower[3]);
  n1sptlineLow->SetLineColor(kRed);
  n1sptlineLow->Draw();
  TLine *n1sptlineHigh = new TLine(0,paramsupper[3],120,paramsupper[3]);
  n1sptlineHigh->SetLineColor(kRed);
  n1sptlineHigh->Draw();

  csigma1s->cd();
  hsigma1spt->SetXTitle("E_T^{HF}");
  hsigma1spt->GetYaxis()->SetRangeUser(plotlimlower[0],plotlimupper[0]);
  hsigma1spt->Draw();
  //hsigma1spt->Fit("pol1");
  TLine *sigmaptlineLow = new TLine(0,paramslower[0],120,paramslower[0]);
  sigmaptlineLow->SetLineColor(kRed);
  sigmaptlineLow->Draw();
  TLine *sigmaptlineHigh = new TLine(0,paramsupper[0],120,paramsupper[0]);
  sigmaptlineHigh->SetLineColor(kRed);
  sigmaptlineHigh->Draw();

  cx1s->cd();
  hx1spt->SetXTitle("E_T^{HF}");
  hx1spt->GetYaxis()->SetRangeUser(plotlimlower[1],plotlimupper[1]);
  hx1spt->Draw();
  //hx1spt->Fit("pol1");
  TLine *x1sptlineLow = new TLine(0,paramslower[1],120,paramslower[1]);
  x1sptlineLow->SetLineColor(kRed);
  x1sptlineLow->Draw();
  TLine *x1sptlineHigh = new TLine(0,paramsupper[1],120,paramsupper[1]);
  x1sptlineHigh->SetLineColor(kRed);
  x1sptlineHigh->Draw();

  cerrmu->cd();
  herrmupt->SetXTitle("E_T^{HF}");
  herrmupt->GetYaxis()->SetRangeUser(plotlimlower[6],plotlimupper[6]);
  herrmupt->Draw();
  TLine *errmuptlineLow = new TLine(0,paramslower[6],120,paramslower[6]);
  errmuptlineLow->SetLineColor(kRed);
  errmuptlineLow->Draw();
  TLine *errmuptlineHigh = new TLine(0,paramsupper[6],120,paramsupper[6]);
  errmuptlineHigh->SetLineColor(kRed);
  errmuptlineHigh->Draw();

  cerrsigma->cd();
  herrsigmapt->SetXTitle("E_T^{HF}");
  herrsigmapt->GetYaxis()->SetRangeUser(plotlimlower[5],plotlimupper[5]);
  herrsigmapt->Draw();
  TLine *errsigmaptlineLow = new TLine(0,paramslower[5],120,paramslower[5]);
  errsigmaptlineLow->SetLineColor(kRed);
  errsigmaptlineLow->Draw();
  TLine *errsigmaptlineHigh = new TLine(0,paramsupper[5],120,paramsupper[5]);
  errsigmaptlineHigh->SetLineColor(kRed);
  errsigmaptlineHigh->Draw();

  clambda->cd();
  hlambdapt->SetXTitle("E_T^{HF}");
  hlambdapt->GetYaxis()->SetRangeUser(plotlimlower[7],plotlimupper[7]);
  hlambdapt->Draw();
  TLine *lambdaptlineLow = new TLine(0,paramslower[7],120,paramslower[7]);
  lambdaptlineLow->SetLineColor(kRed);
  lambdaptlineLow->Draw();
  TLine *lambdaptlineHigh = new TLine(0,paramsupper[7],120,paramsupper[7]);
  lambdaptlineHigh->SetLineColor(kRed);
  lambdaptlineHigh->Draw();

  cnSig1s->cd();
  //gPad->SetLogy();
  hnSig1spt->SetXTitle("E_T^{HF}");
  hnSig1spt->GetYaxis()->SetRangeUser(0,1600*scale);
  hnSig1spt->Draw();
  //hnSig1spt->Fit("pol1");
  TLine *nSig1sptlineLow = new TLine(0,0,120,0);
  nSig1sptlineLow->SetLineColor(kRed);
  nSig1sptlineLow->Draw();

  cnSig2s->cd();
  //gPad->SetLogy();
  hnSig2spt->SetXTitle("E_T^{HF}");
  hnSig2spt->GetYaxis()->SetRangeUser(-20,600*scale);
  hnSig2spt->Draw();
  //hnSig2spt->Fit("pol1");
  TLine *nSig2sptlineLow = new TLine(0,-20,120,-20);
  nSig2sptlineLow->SetLineColor(kRed);
  nSig2sptlineLow->Draw();

  cnSig3s->cd();
  //gPad->SetLogy();
  hnSig3spt->SetXTitle("E_T^{HF}");
  hnSig3spt->GetYaxis()->SetRangeUser(-50,300*scale);
  hnSig3spt->Draw();
  //hnSig3spt->Fit("pol1");
  TLine *nSig3sptlineLow = new TLine(0,-50,120,-50);
  nSig3sptlineLow->SetLineColor(kRed);
  nSig3sptlineLow->Draw();

  cnBkg->cd();
  //gPad->SetLogy();
  hnBkgpt->SetXTitle("E_T^{HF}");
  hnBkgpt->GetYaxis()->SetRangeUser(10,10000*scale);
  hnBkgpt->Draw();
  //hnBkgpt->Fit("pol1");
  TLine *nBkgptlineLow = new TLine(0,0,120,0);
  ...
  //save plots
  calpha->SaveAs(Form("ParameterPlots/PAfitted_alpha_%isbins_HF_y%.2f-%.2f.png",whichUpsilon,yLow,yHigh));
  cf1s->SaveAs(Form("ParameterPlots/PAfitted_f1s_%isbins_HF_y%.2f-%.2f.png",whichUpsilon,yLow,yHigh));
  cmass->SaveAs(Form("ParameterPlots/PAfitted_mass_%isbins_HF_y%.2f-%.2f.png",whichUpsilon,yLow,yHigh));
  cn1s->SaveAs(Form("ParameterPlots/PAfitted_n1s_%isbins_HF_y%.2f-%.2f.png",whichUpsilon,yLow,yHigh));
  csigma1s->SaveAs(Form("ParameterPlots/PAfitted_sigma1s_%isbins_HF_y%.2f-%.2f.png",whichUpsilon,yLow,yHigh));
  cx1s->SaveAs(Form("ParameterPlots/PAfitted_x1s_%isbins_HF_y%.2f-%.2f.png",whichUpsilon,yLow,yHigh));
  cerrmu->SaveAs(Form("ParameterPlots/PAfitted_errmu_%isbins_HF_y%.2f-%.2f.png",whichUpsilon,yLow,yHigh));
  cerrsigma->SaveAs(Form("ParameterPlots/PAfitted_errsigma_%isbins_HF_y%.2f-%.2f.png",whichUpsilon,yLow,yHigh));
  clambda->SaveAs(Form("ParameterPlots/PAfitted_lambda_%isbins_HF_y%.2f-%.2f.png",whichUpsilon,yLow,yHigh));
  cnSig1s->SaveAs(Form("ParameterPlots/PAfitted_nSig1s_%isbins_HF_y%.2f-%.2f.png",whichUpsilon,yLow,yHigh));
  cnSig2s->SaveAs(Form("ParameterPlots/PAfitted_nSig2s_%isbins_HF_y%.2f-%.2f.png",whichUpsilon,yLow,yHigh));
  cnSig3s->SaveAs(Form("ParameterPlots/PAfitted_nSig3s_%isbins_HF_y%.2f-%.2f.png",whichUpsilon,yLow,yHigh));
  cnBkg->SaveAs(Form("ParameterPlots/PAfitted_nBkg_%isbins_HF_y%.2f-%.2f.png",whichUpsilon,yLow,yHigh));

  //save pdf plots
  calpha->SaveAs(Form("ParameterPlots/PAfitted_alpha_%isbins_HF_y%.2f-%.2f.pdf",whichUpsilon,yLow,yHigh));
  cf1s->SaveAs(Form("ParameterPlots/PAfitted_f1s_%isbins_HF_y%.2f-%.2f.pdf",whichUpsilon,yLow,yHigh));
  cmass->SaveAs(Form("ParameterPlots/PAfitted_mass_%isbins_HF_y%.2f-%.2f.pdf",whichUpsilon,yLow,yHigh));
  cn1s->SaveAs(Form("ParameterPlots/PAfitted_n1s_%isbins_HF_y%.2f-%.2f.pdf",whichUpsilon,yLow,yHigh));
  csigma1s->SaveAs(Form("ParameterPlots/PAfitted_sigma1s_%isbins_HF_y%.2f-%.2f.pdf",whichUpsilon,yLow,yHigh));
  cx1s->SaveAs(Form("ParameterPlots/PAfitted_x1s_%isbins_HF_y%.2f-%.2f.pdf",whichUpsilon,yLow,yHigh));
  cerrmu->SaveAs(Form("ParameterPlots/PAfitted_errmu_%isbins_HF_y%.2f-%.2f.pdf",whichUpsilon,yLow,yHigh));
  cerrsigma->SaveAs(Form("ParameterPlots/PAfitted_errsigma_%isbins_HF_y%.2f-%.2f.pdf",whichUpsilon,yLow,yHigh));
  clambda->SaveAs(Form("ParameterPlots/PAfitted_lambda_%isbins_HF_y%.2f-%.2f.pdf",whichUpsilon,yLow,yHigh));
  cnSig1s->SaveAs(Form("ParameterPlots/PAfitted_nSig1s_%isbins_HF_y%.2f-%.2f.pdf",whichUpsilon,yLow,yHigh));
  cnSig2s->SaveAs(Form("ParameterPlots/PAfitted_nSig2s_%isbins_HF_y%.2f-%.2f.pdf",whichUpsilon,yLow,yHigh));
  cnSig3s->SaveAs(Form("ParameterPlots/PAfitted_nSig3s_%isbins_HF_y%.2f-%.2f.pdf",whichUpsilon,yLow,yHigh));
  cnBkg->SaveAs(Form("ParameterPlots/PAfitted_nBkg_%isbins_HF_y%.2f-%.2f.pdf",whichUpsilon,yLow,yHigh));
/*
