...
void FitData_Jaebeom_edited(
       int collId = kPADATA,  
       float ptLow=0, float ptHigh=30, 
       float yLow=0.8, float yHigh=1.93,
       ...
       )
{
  ...
  TFile* f1;
  TFile* f2;
  if      ( collId == kPPDATA) f1 = new TFile("../../yskimPP_L1DoubleMu0PD_Trig-L1DoubleMu0_OpSign_20177262158_.root");
  ...
  RooRealVar alpha1s_1("alpha1s_1","tail shift", 2.071 , 1.593, 3.322);
  ...
  RooRealVar n1s_1("n1s_1","power order", 2.235 , 1.590, 3.237);
  ...
  if (whichModel) {
    c1->SaveAs(Form("NewNominalsTest/altfitresults_upsilon_%s.png",kineLabel.Data()));
    c1->SaveAs(Form("NewNominalsTest/altfitresults_upsilon_%s.pdf",kineLabel.Data()));
  }
  else {
    c1->SaveAs(Form("NewNominalsTest/nomfitresults_upsilon_%s.png",kineLabel.Data()));
    c1->SaveAs(Form("NewNominalsTest/nomfitresults_upsilon_%s.pdf",kineLabel.Data()));
  }
...
TString outFileName;
  if (whichModel){
    outFileName = Form("NewNominalsTest/altfitresults_upsilon_%s.root",kineLabel.Data());
  }
  else {
    outFileName = Form("NewNominalsTest/nomfitresults_upsilon_%s.root",kineLabel.Data());
  }
