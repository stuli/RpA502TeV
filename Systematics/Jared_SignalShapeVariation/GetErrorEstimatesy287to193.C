...
using namespace std;
void GetErrorEstimatesy287to193(int whichUpsilon = 1) {
  ...
  TString outFileName = Form("ErrorEstimates/SystematicErrorSignal%is_y287to193.root",whichUpsilon);
  ...
  if (PPtoo) {
    ...
    if (whichUpsilon==1) ntuple1sy->Fill(binLow,binHigh,temperr);
    else if (whichUpsilon==2) ntuple2sy->Fill(binLow,binHigh,temperr);
    else if (whichUpsilon==3) ntuple3sy->Fill(binLow,binHigh,temperr);
