...
void onia2ySkim_pa_1st_Jared_with_SumET_HF_and_Ntracks( int nevt = -1,
		 int fileID = kPADATA,
		 ...
		 ) 
{
  ...
  if (fileID == kPPDATA) {
    fname = "OniaTree_DoubleMu_Run2015E-PromptReco-v1_Run_262157_262328.root";
  ...
  if (fileID == kPADATA) {
    ...
    fname = "RD2013_pa_1st_run_merged.root";
    mytree->Add(fname.Data());
  }
  ...
  TFile* newfile;
  if (fileID == kPPDATA) {
    newfile = new TFile(Form("Jared_yskimPP_L1DoubleMu0PD_%s_%s_%s_with_SumET_HF_and_Ntracks.root",fdimusign.Data(), getDayAndTime().Data(), skimVersion.Data() ),"recreate");   
  }
  if (fileID == kPADATA) {
    newfile = new TFile(Form("Jared_yskimPA1st_%s_%s_%s_with_SumET_HF_and_Ntracks.root",fdimusign.Data(), getDayAndTime().Data(), skimVersion.Data() ),"recreate");   
  }
  ...
  RooRealVar* hfsum = new RooRealVar("hfsum","HF energy",0,2000,"GeV/c^{2}");
  RooRealVar* ntracks = new RooRealVar("ntracks","Number of tracks",0,2000,"");
  RooArgSet* argSet    = new RooArgSet(*massVar, *ptVar, *yVar, *pt1Var, *pt2Var, *eta1Var, *eta2Var, *hfsum, *ntracks);
  ...
  for(int iev=0; iev<nevt ; ++iev)
  {
    ...
    for (Int_t irqq=0; irqq<Reco_QQ_size; ++irqq) 
    {
      ...
      hfsum->setVal( (double) SumET_HF) ;
      ntracks->setVal( (int) Ntracks) ;
