...
void onia2ySkim_pa_1st( int nevt = -1,
		 int fileID = kPAMCUps1S,
		 ...
		 ) 
{
  ...
  bool isMC = false; 
  if ( (fileID == kPPMC) || (fileID == kPPMCUps1S) || (fileID == kPPMCUps2S) || (fileID == kPPMCUps3S) || (fileID == kAAMC) || (fileID == kAAMCUps1S) || (fileID == kAAMCUps2S) || (fileID == kAAMCUps3S) || (fileID == kPAMCUps1S) || (fileID == kPAMCUps2S) || (fileID == kPAMCUps3S) )
    isMC = true;
  ...
  if (fileID == kPADATA) {
    ...
    fname = "/scratch_menkar/CMS_Trees/OniaTrees_2013_5TeV02_pPb/pPb_MC/OniaTree_MC_Ups1S_PA_5TeV02_WithFSR_tuneD6T.root";
    mytree->Add(fname.Data());
  }
...
//  if (f1->IsZombie()) { cout << "*** KYO : CANNOT open the root file!! Macro terminated ***" << endl; return;} 
  ...
  if (fileID == kPAMCUps1S) {
    newfile = new TFile(Form("SkimmedFiles_Santona/yskimPAMCUps1S_%s_%s_%s.root",fdimusign.Data(), getDayAndTime().Data(), skimVersion.Data() ),"recreate");
  }
