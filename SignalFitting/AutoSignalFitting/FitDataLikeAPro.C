...
double FitDataLikeAPro( 
       ...
       int changeParam=0 //in this macro, 0 is nominal, 1 is change everything.
			) 
{
  ...
  if (changeParam>0) {
    directory = "FitsWithConstraints_changeParams/";
    changealpha = kTRUE;
    changef = kTRUE;
    changen = kTRUE;
    changex = kTRUE;
 ...
 //I use a different skim file for Ntracks bins and HF bins, so that's what this binmode is for. 0=HFMODE, 1=NTMODE.
  ...
  //TString kineLabelIn = getKineLabel (collId, ptLow, ptHigh, -1.93, 1.93, muPtCut, cLow, cHigh, dphiEp2Low, dphiEp2High);
  //TString NomFileName = Form("FitsWithConstraints/nomfitresults_upsilon_%s.root",kineLabelIn.Data());
...
if (whichModel) {
  ...
  if (collId==kPADATA) {
    if (abs((yLow+yHigh)/2+0.01)<1 && yLow>-2) { //mid rapidity
      alphafix = 2.047006;
      ffix = 0.199500;
      nfix = 3.235437;
      xfix = 0.498710;
