...
void SimultaneousFits2ptAndFull( 
       int collId = kPADATA,  
       float ptLow=6, float ptHigh=30, 
       float yLow=0.4, float yHigh=0.8,//Run 1 has p going in -z direction
			...
			) 
{
  ...
  RooDataSet* dsABC = new RooDataSet("dsABC","dsABC",RooArgSet(*(ws->var("mass")), *(ws->var("pt")), *(ws->var("y"))),Index(tp),Import("A",*reducedDS_A),Import("B",*reducedDS_B),Import("C",*reducedDS_C));
...
//A:
  ...
  RooRealVar *nSig1s_A= new RooRealVar("nSig1s_A"," 1S signals",0,1000000);
  RooRealVar *nSig2s_A= new RooRealVar("nSig2s_A"," 2S signals",-20,360000);
  RooRealVar *nSig3s_A= new RooRealVar("nSig3s_A"," 3S signals",-50,260000);
...
//Constraints
  RooFormulaVar *nSig1s_C = new RooFormulaVar("nSig1s_C"," 1S signals","@0+@1",RooArgList(*nSig1s_A,*nSig1s_B) );
  RooFormulaVar *nSig2s_C = new RooFormulaVar("nSig2s_C"," 2S signals","@0+@1",RooArgList(*nSig2s_A,*nSig2s_B) );
  RooFormulaVar *nSig3s_C = new RooFormulaVar("nSig3s_C"," 3S signals","@0+@1",RooArgList(*nSig3s_A,*nSig3s_B) );
  ...
  TString outFileName;
  if (whichModel){
    outFileName = Form("SimultaneousFits/sim_ptSplit_altfitresults_upsilon_%s.root",kineLabel.Data());
  }
  else {
