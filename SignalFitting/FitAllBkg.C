...
void FitAllBkg() {
  ...
  //FitBkg(collId,ptLow,ptHigh,yLow,yHigh,cLow,cHigh,muPtCut,whichModel);
  ...
  for (int whichUpsilon =1; whichUpsilon<=3; whichUpsilon++) {
    ...
    for (int ipt = 0; ipt<numptbins; ipt++) {
      ...
      chisqpt[ipt] = FitBkg(collId,ptLow,ptHigh,yLow,yHigh,cLow,cHigh,muPtCut,whichModel);
    }
    ...
    for (int iy = ystart; iy<numybins; iy++) {
      ...
      cout << "(" << ptLow << "," << ptHigh << "," << yLow << "," << yHigh << ")" << endl;
      chisqy[iy] = FitBkg(collId,ptLow,ptHigh,yLow,yHigh,cLow,cHigh,muPtCut,whichModel);
    }
