...
void SimultaneousChangeToNormal3pt( 
       int collId = kPADATA,
			...
			) 
{
  ...
  if (ext=="_C") {
    depBin = kTRUE;
    if (ptLow==0 && ptHigh==30) {
      ptLowGet = 9;
      ptHighGet = 30;
    }
    else if (ptLow==0 && ptHigh==6) {
      ptLowGet = 4;
      ptHighGet = 6;
    }
    else if (ptLow==6 && ptHigh==30) {
      ptLowGet = 12;
      ptHighGet = 30;
    }
  }
...
if (whichModel) {
  ...
  cb3s = new RooAddPdf("cb3s","Signal 3S",RooArgList(*cb3s_1,*cb3s_2), RooArgList(*f1s) );
    ...
    double nSig3s_init;
  if (depBin) {
    nSig1s_init = Nomws->var("nSig1s_D")->getVal() - Nomws->var("nSig1s_A")->getVal() - Nomws->var("nSig1s_B")->getVal();
    nSig2s_init = Nomws->var("nSig2s_D")->getVal() - Nomws->var("nSig2s_A")->getVal() - Nomws->var("nSig2s_B")->getVal();
    nSig3s_init = Nomws->var("nSig3s_D")->getVal() - Nomws->var("nSig3s_A")->getVal() - Nomws->var("nSig3s_B")->getVal();
  }
  else {
