...
void SimultaneousChangeToNormal2pt(
       int collId = kPADATA,
			...
			) 
{
  ...
  cout << endl << "EXTRACTING FIT IN PT BIN [" << ptLowGet << "," << ptHighGet << "]" << endl << endl;
  ...
  TString kineLabelGet = getKineLabel (collId, ptLowGet, ptHighGet, yLow, yHigh, muPtCut, cLow, cHigh, dphiEp2Low, dphiEp2High);
  TString kineCut = Form("pt>%.2f && pt<%.2f && y>%.2f && y<%.2f && eta1<%.2f && eta1>%.2f && eta2<%.2f && eta2>%.2f",ptLowGet, ptHighGet, yLowLab, yHighLab, eta_high,eta_low, eta_high,eta_low );
  ...
  TString NomFileName = Form("SimultaneousFits/sim_ptSplit3_nomfitresults_upsilon_%s.root",kineLabel.Data());
...
if (whichModel) {
  ...
  double nSig3s_init;
  if (depBin) {
    nSig1s_init = Nomws->var("nSig1s_C")->getVal() - Nomws->var("nSig1s_A")->getVal();
    nSig2s_init = Nomws->var("nSig2s_C")->getVal() - Nomws->var("nSig2s_A")->getVal();
    nSig3s_init = Nomws->var("nSig3s_C")->getVal() - Nomws->var("nSig3s_A")->getVal();
  }
  else {
  ...
  if (depBin) {
    ...
    TString FullBinFileName = Form("SimultaneousFits/Normal/nomfitresults_upsilon_%s.root",kineLabel.Data());
    TFile* FullBinFile = TFile::Open(FullBinFileName,"READ");
    RooWorkspace *FullBinws = (RooWorkspace*)FullBinFile->Get("workspace");
    FullBinFile->Close("R");
    double err1C = FullBinws->var("nSig1s")->getError();
    double err2C = FullBinws->var("nSig2s")->getError();
    double err3C = FullBinws->var("nSig3s")->getError();
    cout << "err1A = " << err1A << endl;
    cout << "err1C = " << err1C << endl;
    nSig1s->setError(TMath::Sqrt(err1C*err1C - err1A*err1A));
    nSig2s->setError(TMath::Sqrt(err2C*err2C - err2A*err2A));
    nSig3s->setError(TMath::Sqrt(err3C*err3C - err3A*err3A));    
  }
  else {
  ...
  float text_size = 19;
  int text_color = 1;
  if(ptLowGet==0 && ptHighGet!=2.5) drawText(Form("p_{T}^{#mu#mu} < %.f GeV/c",ptHighGet ),pos_text_x,pos_text_y,text_color,text_size);
  else if(ptLowGet == 2.5 && ptHighGet==5) drawText(Form("%.1f < p_{T}^{#mu#mu} < %.f GeV/c",ptLowGet,ptHighGet ),pos_text_x,pos_text_y,text_color,text_size);
  else if(ptLowGet == 0 && ptHighGet==2.5) drawText(Form("p_{T}^{#mu#mu} < %.1f GeV/c",ptHighGet ),pos_text_x,pos_text_y,text_color,text_size);
  else drawText(Form("%.f < p_{T}^{#mu#mu} < %.f GeV/c",ptLowGet,ptHighGet ),pos_text_x,pos_text_y,text_color,text_size);
  ...
  cout << "3S signal    =  " << outh->GetBinContent(3) << " +/- " << outh->GetBinError(3) << endl;
	...
	cout << "if ( binMatched( "<<muPtCut<<",  " << ptLowGet <<", "<< ptHighGet<<", "<< yLow<<", "<< yHigh << ", " << cLow << ", " << cHigh << ") ) " ; 
  cout << "  { setSignalParMC( " ;
