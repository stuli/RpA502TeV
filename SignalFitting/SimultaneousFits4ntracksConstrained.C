...
void SimultaneousFits4ntracksConstrained( 
       int collId = kPADATA,  
			...
			) 
{
  ...
  int ntLow_A, ntHigh_A, ntLow_B, ntHigh_B, ntLow_C, ntHigh_C, ntLow_D, ntHigh_D;
  ntLow_A = 0;
  ntHigh_A = 40;
  ntLow_B = 40;
  ntHigh_B = 62;
  ...
  cout << endl << "PERFORMING SIMULTANEOUS FIT IN NTRACKS BINS [" << ntLow_A << "," << ntHigh_A << "], [" << ntLow_B << "," << ntHigh_B << "], [" << ntLow_C << "," << ntHigh_C << "], [" << ntLow_D << "," << ntHigh_D << "]" << endl << endl;

  //Select Data Set
  if (collId==kPADATA) {
    hfntracksCut_A = Form(" && ntracks>=%i && ntracks<%i", ntLow_A, ntHigh_A );
    hfntracksCut_B = Form(" && ntracks>=%i && ntracks<%i", ntLow_B, ntHigh_B );
    hfntracksCut_C = Form(" && ntracks>=%i && ntracks<%i", ntLow_C, ntHigh_C );
    hfntracksCut_D = Form(" && ntracks>=%i && ntracks<%i", ntLow_D, ntHigh_D );
...
//Constraints
  ...
  RooAddPdf* model_D = new RooAddPdf("model_D","1S+2S+3S + Bkg",RooArgList(*cb1s_D, *cb2s_D, *cb3s_D, *bkg_D),RooArgList(*nSig1s_D,*nSig2s_D,*nSig3s_D,*nBkg_D));
  ws->import(*model_D);
  c_D->cd();
  ...
  TString outFileName;
  if (whichModel){
    outFileName = Form("SimultaneousFits/sim_ntSplit4_altfitresults_upsilon_%s.root",kineLabel.Data());
  }
  else {
    outFileName = Form("SimultaneousFits/sim_ntSplit4_nomfitresults_upsilon_%s.root",kineLabel.Data());
  }
