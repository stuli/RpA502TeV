...
double FitDataWithInternalConstraintsHFNtracks( 
       int collId = kPADATA,  
			...
			) 
{
  ...
  if (changen) directory = "FitsWithConstraints_changen/";
  if (changex) directory = "FitsWithConstraints_changex/";
  if (changealpha) directory = "FitsWithConstraints_changealpha/";
  if (changef) directory = "FitsWithConstraints_changef/";
  ...
  TString NomFileName = Form("FitsWithConstraints/nomfitresults_upsilon_%s.root",kineLabel.Data());
  kineLabel = kineLabel + Form("_hfsum%.2f-%.2f_ntracks%i-%i",hfLow, hfHigh, ntracksLow, ntracksHigh );
  TString NomFileName = Form("/media/jared/Acer/Users/Jared/Desktop/Ubuntu_Overflow/Fits/NominalFits_2018_07_18/nomfitresults_upsilon_%s.root",kineLabel.Data());
  ...
  if ((sigma1s_1_init>=paramsupper[0]) || (sigma1s_1_init<=paramslower[0])) sigma1s_1_init=0.05;
  double x1s_init = Nomws->var("x1s")->getVal();
  if ((x1s_init>=paramsupper[1]) || (x1s_init<=paramslower[1])) x1s_init=0.5;
  double alpha1s_1_init = Nomws->var("alpha1s_1")->getVal();
  if ((alpha1s_1_init>=paramsupper[2]) || (alpha1s_1_init<=paramslower[2])) alpha1s_1_init=2.5;
  ...
  if ((f1s_init>=paramsupper[4]) || (f1s_init<=paramslower[4])) f1s_init=0.5;
...
if (whichModel) {
  ...
  if (changen || changex || changealpha || changef) {
  ...
  if (changen) nfix = sysws->var("n1s_1")->getVal();
  if (changex) xfix = sysws->var("x1s")->getVal();
  if (changealpha) alphafix = sysws->var("alpha1s_1")->getVal();
  ...
  if (yLow<0) {
    ...
    TString ppFileName = Form("/media/jared/Acer/Users/Jared/Desktop/Ubuntu_Overflow/Fits/NominalFits_2018_09_25/nomfitresults_upsilon_%s.root",kineLabelpp.Data());
    ...
    //f1s->setVal(ffix);
    if (changef) ffix = sysws->var("f1s")->getVal();
  ...
  if (changen) cout << "alternative n constraint" << endl;
  if (changex) cout << "alternative x constraint" << endl;
  if (changealpha) cout << "alternative alpha constraint" << endl;
  if (changef) cout << "alternative f constraint" << endl;
