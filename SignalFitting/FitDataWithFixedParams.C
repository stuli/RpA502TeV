			...
			) 
{
 ...
 //I use a different skim file for Ntracks bins and HF bins, so that's what this binmode is for. 0=HFMODE, 1=NTMODE.
  ...
  //Select Data Set
  cout << "Selecting Dataset" << endl;
  if (collId==kPADATA) {
    cout << "collId = kPADATA" << endl;
    if (binmode==0) {
      cout << "binmode = 0" << endl;
  ...
  if (binmode>0) kineCut = kineCut + hfntracksCut;
  ...
  //TString NomFileName = Form("RoundFits_R1b/nomfitresults_upsilon_%s.root",kineLabel.Data());
  cout << "Opening " << NomFileName << endl;
  TFile* NomFile = TFile::Open(NomFileName,"READ");
  cout << "Successfully opened " << NomFileName << endl;
  ...
  //randomize ICs:
  if (randomSeeds) {
    cout << "Randomizing seeds." << endl;
    double rangeScale = 0.8;
    TRandom3 rnd3(0);
  ...
  cout << "Setting up crystal ball shapes" << endl;
...
if (whichModel) {
  ...
  //BACKGROUND
  cout << "building background pdf" << endl;
  double err_mu_init = 10;
  double err_sigma_init = 1;
  double m_lambda_init = 15;
  //if (ptLow<5 || ptHigh<10) {
  if (ptLow<5) {
  ...
  if  (ptLow >= 5.0 && ptHigh >= 10.0)        bkg = bkgHighPt ;
  else bkg = bkgLowPt;
  ...
  if (whichRound==R1a) {
    if (abs((yLow+yHigh)/2+0.01)<1) {
      alpha1s_1.setVal(2.082702); //mid rapidity
      alpha1s_1.setConstant();
    }
    else {
      alpha1s_1.setVal(2.001927); //extreme rapidity
      alpha1s_1.setConstant();
    }
  }
  else if (whichRound==R1b) {
    if (abs((yLow+yHigh)/2+0.01)<1) {
      n1s_1.setVal(2.566799); //mid rapidity
      n1s_1.setConstant();
    }
    else {
      n1s_1.setVal(3.133960); //extreme rapidity
      n1s_1.setConstant();
    }
  }
  else if (whichRound==R2a) {
    if (abs((yLow+yHigh)/2+0.01)<1) {
      n1s_1.setVal(3.235437); //mid rapidity
      n1s_1.setConstant();
    }
    else {
      n1s_1.setVal(1.353564); //extreme rapidity
      n1s_1.setConstant();
    }
  }
  else if (whichRound==R2b) {
    if (abs((yLow+yHigh)/2+0.01)<1) {
      alpha1s_1.setVal(2.084755); //mid rapidity
      alpha1s_1.setConstant();
    }
    else {
      alpha1s_1.setVal(2.124860); //extreme rapidity
      alpha1s_1.setConstant();
    }
  }
  else if (whichRound==R3a) {
    if (abs((yLow+yHigh)/2+0.01)<1) {
      alpha1s_1.setVal(2.047006); //mid rapidity
      alpha1s_1.setConstant();
      n1s_1.setVal(3.235437); //mid rapidity
      n1s_1.setConstant();
    }
    else {
      alpha1s_1.setVal(1.725369); //extreme rapidity
      alpha1s_1.setConstant();
      n1s_1.setVal(1.353564); //extreme rapidity
      n1s_1.setConstant();
    }
    ...
    if (abs((yLow+yHigh)/2+0.01)<1) {
      alpha1s_1.setVal(2.084755); //mid rapidity
      alpha1s_1.setConstant();
      n1s_1.setVal(3.503885); //mid rapidity
      n1s_1.setConstant();
    }
    else {
      alpha1s_1.setVal(2.124860); //extreme rapidity
      alpha1s_1.setConstant();
      n1s_1.setVal(1.877690); //extreme rapidity
      n1s_1.setConstant();
    }
  }
  ...
  cout << "Round " << roundLabel[whichRound].Data() << endl;
