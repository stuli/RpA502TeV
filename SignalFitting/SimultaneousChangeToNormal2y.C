...
void SimultaneousChangeToNormal2y( 
       int collId = kPADATA,
			...
			) 
{
  ...
  cout << endl << "EXTRACTING FIT IN BIN pt[" << ptLow << "," << ptHigh << "]y[" << yLowGet << "," << yHighGet << "]" << endl << endl;
  ...
  if (collId==kPADATA) {
    ...
    yLowLab = yLow+0.47;
    yHighLab = yHigh+0.47;
    yLowLabGet = yLowGet+0.47;
    yHighLabGet = yHighGet+0.47;
  }
    ...
    yHighLab = yHigh;
    yLowLabGet = yLowGet;
    yHighLabGet = yHighGet;
  }

  //Define cuts
  TString kineLabel = getKineLabel (collId, ptLow, ptHigh, yLow, yHigh, muPtCut, cLow, cHigh, dphiEp2Low, dphiEp2High);
  TString kineLabelGet = getKineLabel (collId, ptLow, ptHigh, yLowGet, yHighGet, muPtCut, cLow, cHigh, dphiEp2Low, dphiEp2High);
  TString kineCut = Form("pt>%.2f && pt<%.2f && y>%.2f && y<%.2f && eta1<%.2f && eta1>%.2f && eta2<%.2f && eta2>%.2f",ptLow, ptHigh, yLowLabGet, yHighLabGet, eta_high,eta_low, eta_high,eta_low );
  ...
  TString NomFileName = Form("SimultaneousFits/sim_ySplit_nomfitresults_upsilon_%s.root",kineLabel.Data());
...
if (whichModel) {
  ...
  if(yLowGet==0) drawText(Form("|y^{#mu#mu}| < %.2f",yHighGet ), pos_text_x,pos_text_y-pos_y_diff,text_color,text_size);
  else drawText(Form("%.2f < y^{#mu#mu} < %.2f",yLowGet,yHighGet ), pos_text_x,pos_text_y-pos_y_diff,text_color,text_size);    // for pPb
  ...
  cout << "3S signal    =  " << outh->GetBinContent(3) << " +/- " << outh->GetBinError(3) << endl;
	...
	cout << "if ( binMatched( "<<muPtCut<<",  " << ptLow <<", "<< ptHigh<<", "<< yLowGet<<", "<< yHighGet << ", " << cLow << ", " << cHigh << ") ) " ; 
  cout << "  { setSignalParMC( " ;
