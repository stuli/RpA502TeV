			...
			) 
{
  ...
  double paramsupper[8] = {0.2, 1.0, 6.0, 5.0, 1.0, 15.0, 15.0, 25.0};
  ...
  TString NomFileName = Form("MCFits/MC1Snomfitresults_upsilon_%s.root",kineLabel.Data());
...
if (whichModel) {
  ...
  //import fitted same-sign data
  delete Nomws;
  delete NomFile;
  cout << "Importing workspace" << endl;
  NomFileName = Form("BkgFits/Bkgnomfitresults_upsilon_%s.root",kineLabel.Data());
  cout << NomFileName << endl;
  TFile* NomFile = TFile::Open(NomFileName,"READ");
  RooWorkspace *Nomws = (RooWorkspace*)NomFile->Get("workspace");
  NomFile->Close("R");
  ...
  RooRealVar *nBkg = new RooRealVar("nBkg","fraction of component 1 in bkg",0,5000000);
  ...
  TString directory = "FitsWithMCSeeds/";
  if (whichModel) {
