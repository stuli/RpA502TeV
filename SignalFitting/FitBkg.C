			...
			) 
{
  ...
  //Select Data Set
  if (collId==kPADATA) {
    f1 = new TFile("/media/jared/Acer/Users/Jared/Desktop/Ubuntu_Overflow/DataTrees/Jared_yskimPA1st_SSign_20182261712_unIdentified.root");
    f2 = new TFile("/media/jared/Acer/Users/Jared/Desktop/Ubuntu_Overflow/DataTrees/Jared_yskimPA2nd_SSign_20182261756_unIdentified.root");
    yLowLab = yLow+0.47;
    yHighLab = yHigh+0.47;
    ...
    f1 = new TFile("/media/jared/Acer/Users/Jared/Desktop/Ubuntu_Overflow/DataTrees/");
    yLowLab = yLow;
    yHighLab = yHigh;
...
*/
  ...
  //Build the model
  RooAddPdf* model = new RooAddPdf("model","1S+2S+3S + Bkg",RooArgList(*bkg),RooArgList(*nBkg));
  ...
  TString directory = Form("BkgFits/");
  if (whichModel) {
    c1->SaveAs(Form("%sBkgaltfitresults_upsilon_%s.png",directory.Data(),kineLabel.Data()));
    c1->SaveAs(Form("%sBkgaltfitresults_upsilon_%s.pdf",directory.Data(),kineLabel.Data()));
  }
  else {
    c1->SaveAs(Form("%sBkgnomfitresults_upsilon_%s.png",directory.Data(),kineLabel.Data()));
    c1->SaveAs(Form("%sBkgnomfitresults_upsilon_%s.pdf",directory.Data(),kineLabel.Data()));
  }
  ...
  float temp1 = ws->var("nBkg")->getVal();  
  float temp1err = ws->var("nBkg")->getError();   
  
  outh->SetBinContent(1,  temp1 ) ;
  outh->SetBinError  (1,  temp1err ) ;

  cout << "Background    =  " << outh->GetBinContent(1) << " +/- " << outh->GetBinError(1) << endl;

	cout << "if ( binMatched( "<<muPtCut<<",  " << ptLow <<", "<< ptHigh<<", "<< yLow<<", "<< yHigh << ", " << cLow << ", " << cHigh << ") ) " ; 

TString outFileName;
  if (whichModel){
    outFileName = Form("%sBkgaltfitresults_upsilon_%s.root",directory.Data(),kineLabel.Data());
  }
  else {
    outFileName = Form("%sBkgnomfitresults_upsilon_%s.root",directory.Data(),kineLabel.Data());
  }
