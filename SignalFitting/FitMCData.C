			...
			) 
{
  ...
  int   nMassBin  = (massHigh-massLow)*200;
  ...
  float yLowUse = yLow;
  float yHighUse = yHigh;
  cout << "yLow = " << yLow << endl;
  cout << "yLowUse = " << yLowUse << endl;
 /* if (yLow<-2.0) {
    yLowUse = -1.93;
    cout << "yLowUse = " << yLowUse << endl;
    yHighUse = abs(yLow);
  ...
  if (yLow<0) {
    yHighUse = abs(yLow);
    yLowUse = abs(yHigh);
    if (yLow<-1.9 && yHigh>1.9) {
      yLowUse = 0.0;
      yHighUse = yHigh;
    }
  }*/
  TString kineLabelUse = getKineLabel (kPADATA, ptLow, ptHigh, yLowUse, yHighUse, muPtCut, cLow, cHigh, dphiEp2Low, dphiEp2High);
...
if (whichModel) {
 ...
 /* double err_mu_init = 8;
  ...
  //fix parameters
  float nfix;
  if (collId==kPADATA) nfix = 2.73377;
  else if (collId==kPPDATA) nfix = 1.46701;
  //n1s_1.setVal(nfix);
  //n1s_1.setConstant();
  //alpha1s_1.setVal(2.8);
  //alpha1s_1.setConstant();

  //Build the model
  RooAddPdf* model;
  if (whichUpsilon==1) {
    model = new RooAddPdf("model","1S+2S+3S + Bkg",RooArgList(*cb1s),RooArgList(*nSig1s));
  }
  else if (whichUpsilon==2) {
    model = new RooAddPdf("model","1S+2S+3S + Bkg",RooArgList(*cb2s),RooArgList(*nSig2s));
  }
  else if (whichUpsilon==3) {
    model = new RooAddPdf("model","1S+2S+3S + Bkg",RooArgList(*cb3s),RooArgList(*nSig3s));
  }
  ws->import(*model);
...
/*  float pos_text_x_params = 0.7;
  ...
  int numParams = 13;
  int numfixed;
  if (collId==kPADATA) numfixed = 4;
  else if (collId==kPPDATA) numfixed = 3;
  if (ptLow>5) {
    numParams = 11;
    TString paramNames[11] = {"n1s_1","alpha1s_1","x1s","f1s","sigma1s_1","m_{#Upsilon(1S)}","nSig1s","nSig2s","nSig3s"};
  }
  else {
    TString paramNames[13] = {"n1s_1","alpha1s_1","x1s","f1s","sigma1s_1","m_{#Upsilon(1S)}","nSig1s","nSig2s","nSig3s"};
  }
  for (int iparam = 0; iparam<numParams; iparam++) {
    TString paramText;
    paramText = Form("%s = %.2f #pm %.2f", paramNames[iparam].Data(), ws->var(paramNames[iparam].Data())->getVal(), ws->var(paramNames[iparam].Data())->getError());
  ...
  if (whichModel) {
    c1->SaveAs(Form("MCFits/MC%iSaltfitresults_upsilon_%s.png",whichUpsilon,kineLabel.Data()));
    c1->SaveAs(Form("MCFits/MC%iSaltfitresults_upsilon_%s.pdf",whichUpsilon,kineLabel.Data()));
  }
  else {
    c1->SaveAs(Form("MCFits/MC%iSnomfitresults_upsilon_%s.png",whichUpsilon,kineLabel.Data()));
    c1->SaveAs(Form("MCFits/MC%iSnomfitresults_upsilon_%s.pdf",whichUpsilon,kineLabel.Data()));
  }
  ...
  cout << Form("%iS signal    =  ",whichUpsilon) << outh->GetBinContent(1) << " +/- " << outh->GetBinError(1) << endl;
...
TString outFileName;
  if (whichModel){
    outFileName = Form("MCFits/MC%iSaltfitresults_upsilon_%s.root",whichUpsilon,kineLabel.Data());
  }
  else {
    outFileName = Form("MCFits/MC%iSnomfitresults_upsilon_%s.root",whichUpsilon,kineLabel.Data());
  }
