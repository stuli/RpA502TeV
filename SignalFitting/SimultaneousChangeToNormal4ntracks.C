...
void SimultaneousChangeToNormal4ntracks( 
       int collId = kPADATA,
			...
			) 
{
  ...
  cout << endl << "EXTRACTING FIT IN NTRACKS BIN [" << ntLowGet << "," << ntHighGet << "]" << endl << endl;
  ...
  kineLabelGet = kineLabelGet + Form("_hfsum%.2f-%.2f_ntracks%i-%i",0.00, 120.00, ntLowGet, ntHighGet );
  ...
  TString hfntracksCut = Form(" && ntracks>=%i && ntracks<%i", ntLowGet, ntHighGet );
  ...
  TString NomFileName = Form("SimultaneousFits/sim_ntSplit4_nomfitresults_upsilon_%s.root",kineLabel.Data());
