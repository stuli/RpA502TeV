...
double FitDataWithInternalConstraints( 
       int collId = kPADATA,
       float ptLow=2, float ptHigh=4,
			...
			) 
{
  ...
  TString NomFileName = Form("FitsWithConstraints_changealpha_fixedSignal/nomfitresults_upsilon_%s.root",kineLabel.Data());
...
if (whichModel) {
  ...
  if (collId==kPADATA) {
    ...
    if (changen) nfix = 1.46701;
    if (changex) xfix = 0.507152;
    if (changealpha) alphafix = 1.90773;
  }
  //n1s_1.setVal(nfix);
  //x1s->setVal(xfix);
