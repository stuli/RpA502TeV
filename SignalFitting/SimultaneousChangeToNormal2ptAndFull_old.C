...
void SimultaneousChangeToNormal2ptAndFull( 
       int collId = kPADATA,
			...
			) 
{
  ...
  TString NomFileName = Form("SimultaneousFits/sim_ptSplit_nomfitresults_upsilon_%s.root",kineLabel.Data());
...
if (whichModel) {
  ...
  cb3s = new RooAddPdf("cb3s","Signal 3S",RooArgList(*cb3s_1,*cb3s_2), RooArgList(*f1s) );
    ...
    double nSig3s_init;
  if (FullBin) {
    nSig1s_init = Nomws->var("nSig1s_A")->getVal() + Nomws->var("nSig1s_B")->getVal();
    nSig2s_init = Nomws->var("nSig2s_A")->getVal() + Nomws->var("nSig2s_B")->getVal();
    nSig3s_init = Nomws->var("nSig3s_A")->getVal() + Nomws->var("nSig3s_B")->getVal();
  }
  else {
  ...
  if (FullBin) {
    ...
    nSig1s->setError(TMath::Sqrt(err1A*err1A + err1B*err1B));
    nSig2s->setError(TMath::Sqrt(err2A*err2A + err2B*err2B));
    nSig3s->setError(TMath::Sqrt(err3A*err3A + err3B*err3B));
  }
  else {
  ...
  cout << "nSig1sErr = " << nSig1s->getError() << endl;
  cout << "nSig2sErr = " << nSig2s->getError() << endl;
  cout << "nSig3sErr = " << nSig3s->getError() << endl;
  cout << "Root would say it's" << endl;
  cout << "nSig1sErr = " << Nomws->var("nSig1s_C")->getError() << endl;
  cout << "nSig2sErr = " << Nomws->var("nSig2s_C")->getError() << endl;
  cout << "nSig3sErr = " << Nomws->var("nSig3s_C")->getError() << endl;
