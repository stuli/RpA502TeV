...
int draw_bkgparam_pt_comp(TString szAA = "PP", int states =3, int DrawOpt = 2) 
{
  setTDRStyle();
...
*/
  ...
  c_lambda->cd();
  for(int ifit=0; ifit<nFit; ifit++){ h1_lambda[0]->Draw("pe"); if(ifit>0) h1_lambda[ifit]->Draw("pe same");}
