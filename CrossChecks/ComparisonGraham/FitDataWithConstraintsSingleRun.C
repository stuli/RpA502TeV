...
void FitDataWithConstraintsSingleRun( 
	   int runNumber = 1,
       int collId = kPADATA,
			...
			) 
{
  ...
  TString directory = "";
  if (runNumber==1)
	directory = "Run1FitsConstrained/";
  else if (runNumber==2)
    directory = "Run2FitsConstrained/";
  ...
  if (collId==kPADATA) {
    if (runNumber==1)
	  ...
	  f1 = new TFile("../../../yskimPA2nd_OpSign_20177262044_unIdentified.root");
    yLowLab = yLow+0.47;
    yHighLab = yHigh+0.47;
  ...
  TFile* lowPtBkgFile;
  if (collId == kPADATA)
	lowPtBkgFile = new TFile("../../Systematics/Graham_BkgVariation/altBkgModels.root","READ");
  else if (collId == kPPDATA)
	lowPtBkgFile = new TFile("../../Systematics/Graham_BkgVariation/altBkgModelsPP.root","READ");
