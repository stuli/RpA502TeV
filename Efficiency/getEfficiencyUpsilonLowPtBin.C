...
void getEfficiencyUpsilonLowPtBin(int state = 1, bool useDataWeight=true, float LowPtBin = 6,
				  int trgIdx=0,  int trkIdx=0, int muIdx=0,  int staIdx=0
				  //  id = -100 means no correction on this 
			  ) {  // 1S, 2S, 3S
  TH1::SetDefaultSumw2();
  ...
  TFile* fout = new TFile(Form("%s/efficiency_ups%ds_useDataPtWeight%d_tnp_trgId%d_trkId%d_muId%d_staId%d_LowPtBin%d.root",outputDirName.Data(), state,useDataWeight,trgIdx, trkIdx, muIdx, staIdx, (int)LowPtBin),"recreate");

  c_eff_pt->SaveAs(Form("%s/eff_vs_pt_%ds_useDataPtWeight%d_tnp_trgId%d_trkId%d_muId%d_staId%d_LowPtBin%d.pdf", outputDirName.Data(), state, useDataWeight, trgIdx, trkIdx, muIdx, staIdx, (int)LowPtBin ));
  c_eff_rap->SaveAs(Form("%s/eff_vs_rap_%ds_useDataPtWeight%d_tnp_trgId%d_trkId%d_muId%d_staId%d_LowPtBin%d.pdf", outputDirName.Data(), state, useDataWeight, trgIdx, trkIdx, muIdx, staIdx, (int)LowPtBin ));
  c_eff_cent->SaveAs(Form("%s/eff_vs_cent_%ds_useDataPtWeight%d_tnp_trgId%d_trkId%d_muId%d_staId%d_LowPtBin%d.pdf", outputDirName.Data(), state, useDataWeight, trgIdx, trkIdx, muIdx, staIdx, (int)LowPtBin ));
