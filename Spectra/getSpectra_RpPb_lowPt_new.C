...
void getSpectra_RpPb_lowPt_new(int state = 1) {  
  ...
  hrapAccPA_cross = (TH1D*) infacc->Get(Form("hrapAccCrossPt1_%dS",state)); 
...
*/
  hrapEff = (TH1D*)infeff->Get("EffNomRatRapLowpT");
  hptEff = (TH1D*)infeff->Get("EffNomRatPtRapNeg");
...
/*
  for(int irap = 1; irap<=nYBins_cr; irap++){
    ...
    valErr yieldPA = getYield(state, kPADATA, 0,6, yBin_cr[irap-1],yBin_cr[irap],0,200,0,100);
...
*/
  ...
  for ( int ipt = 1 ; ipt<= nPtBins ; ipt++) {
    ...
    valErr yieldPA = getYield(state, kPADATA, ptBin[ipt-1],ptBin[ipt], -1.93,0.00, 0,200,0,100);
...
//  gCross_pt->SetName("gCross_pt");
  ...
  TFile *wf = new TFile(Form("finalResults/Ups_%d_RPA_lowPt.root",state),"recreate");
  gRPA_rap->Write();
  gRPA_pt->Write();
//  gCross_rap->Write();
//  gCross_pt->Write();
