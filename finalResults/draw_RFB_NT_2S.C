...
//  double relsys = 0.1;
  ...
  double px_1[4] = {50-50/8*1,150-50/8*1,250-50/8*1,350-50/8*1};
  double px_2[4] = {50+50/8*1,150+50/8*1,250+50/8*1,350+50/8*1};

  ////////////////////////////////////////////////////////////////
  //// read input file : value & stat.
  TFile* fIn;
  fIn = new TFile("Ups_2_RFB_Ntracks.root","READ");
  ...
  TFile* fInSys;
  fInSys = new TFile("../Systematics/mergedSys_ups2s_rfb.root","READ");
  TH1D* hSys[nState];
  int npoint[nState] = {4,4};
  for (int is=0; is<nState; is++){
...
//    else {gRPA[is]->Draw("P");}
  ...
  arrLeg->SetLineWidth(2);
    ...
    leg -> AddEntry(gRPA[0],"0 < |y_{CM}| < 0.8","lp");
    leg -> AddEntry(gRPA[1],"0.8 < |y_{CM}| < 1.93","lp");
...
//  globtex->DrawLatex(0.22, sz_init, "p_{T}^{#mu} > 4 GeV/c");
  ...
  globtex->DrawLatex(0.22, sz_init-sz_step*2, "#varUpsilon(2S)");
...
//  globtex->DrawLatex(0.22, sz_init-sz_step*2, "Cent. 0-100%");
  ...
  CMS_lumi_raaCent( c1, iPeriod, iPos );
	...
	c1->Update();
  c1->SaveAs("plots/RFB_vs_nt_2S.pdf");
  c1->SaveAs("plots/RFB_vs_nt_2S.png");
