...
//  double relsys = 0.1;
  ...
  for (int ipt=0; ipt< npoint; ipt++) 
  {
    ...
    pytmp = h_pPb->GetBinContent(ipt+2)/h_pPb->GetBinContent(1);
    pytmp = pytmp/((double)h_pp->GetBinContent(ipt+2)/h_pp->GetBinContent(1));
    extmp=exsys;
    eytmp=0.05;
    gRAA[1]->SetPoint(ipt, cpx_1s_exsys[ipt], pytmp);
    gRAA_sys[1]->SetPoint(ipt, cpx_1s_exsys[ipt], pytmp);
    gRAA[1]->SetPointError(ipt, 0, eytmp);
    gRAA_sys[1]->SetPointError(ipt, extmp, extmp, pytmp*eytmp, pytmp*eytmp);
  }
  ...
  //// legend
  TLegend *leg= new TLegend(0.574, 0.63, 0.864, 0.80);
  SetLegendStyle(leg);
  leg -> SetHeader("");
  //leg -> SetHeader("#Upsilon's");
  leg -> AddEntry(gRAA[0],"HIN-13-003","lp");
  leg -> AddEntry(gRAA[1],"new Result","lp");
...
//  globtex->DrawLatex(0.22, sz_init-sz_step*2, "|#eta^{#mu}| < 2.4");
  ...
  globtex_label->DrawLatex(0.639, sz_init-sz_step*15.24, "#Upsilon(3S)/#Upsilon(1S)");
