			...
			) 
{
  ...
  NomFileName = Form("/media/jared/Acer/Users/Jared/Desktop/Ubuntu_Overflow/Fits/NominalFits_2018_09_25/nomfitresults_upsilon_%s.root",kineLabel.Data());
  ...
  workspace->pdf("model")->plotOn(myPlot2,Name("Sig1S"),Components(RooArgSet(*cb1s)),LineColor(kGreen+2),LineWidth(2),LineStyle(2));
  workspace->pdf("model")->plotOn(myPlot2,Components(RooArgSet(*cb2s)),LineColor(kGreen+2),LineWidth(2),LineStyle(2));
  workspace->pdf("model")->plotOn(myPlot2,Components(RooArgSet(*cb3s)),LineColor(kGreen+2),LineWidth(2),LineStyle(2));
  ...
  myPlot2->Draw();
  RooFitResult* fitRes2 = workspace->obj("fitresult_model_reducedDS");
  fitRes2->Print("v");
