...
void draw_CrossSection_rap_isArrow_run12(int ppAA=2, bool isArrow=false) //1=pp, 2=AA
{
  setTDRStyle();
...
//  double relsys = 0.1;
  ...
  for (int is=0; is<nState; is++){
    ...
    SetGraphStyle(gCrossSection_1[is], is+1, is+1); 
    SetGraphStyleSys(gCrossSection_sys[is], is); 
    SetGraphStyleSys(gCrossSection_sys_1[is], is+1); 
	}
  ...
  */
  ...
  gCrossSection_sys[0]->SetMaximum(0.7);
...
//  gPad->SetLogy(1); // for cross section
  for (int is=0; is<1; is++){
    if ( is==0) {gCrossSection_sys[is]->Draw("A5");gCrossSection_sys_1[is]->Draw("5");}
    else if(is==ulstate && isArrow==true) {
      for(int ipt=0;ipt<n3s;ipt++){
        box68per[ipt]->Draw("l");
      }
      gCrossSection_sys[is]->Draw("5");
      gCrossSection_sys_1[is]->Draw("5");
    }
    else {
      gCrossSection_sys[is]->Draw("5");
      gCrossSection_sys_1[is]->Draw("5");
  ...
  if (isArrow==false) {
    for (int is=0; is<1; is++){
      leg -> AddEntry(gCrossSection[is],Form(" #Upsilon(%dS) Run1",is+1),"lp");
      leg -> AddEntry(gCrossSection_1[is],Form(" #Upsilon(%dS) Run2",is+1),"lp");
    }
  }
  else {
...
//  globtex->DrawLatex(0.27, sz_init-sz_shift-sz_step*2, "|#eta^{#mu}| < 2.4");
  ...
  CMS_lumi( c1, 3, iPos );
	...
	c1->Update();
  c1->SaveAs(Form("Comp_Run12_CrossSection_vs_rap_%s.pdf",sz_ppAA.Data()));
  c1->SaveAs(Form("Comp_Run12_CrossSection_vs_rap_%s.png",sz_ppAA.Data()));
