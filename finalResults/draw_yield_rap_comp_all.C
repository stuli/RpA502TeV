...
int draw_yield_rap_comp_all(TString szAA = "PA", int states =1) 
{
  setTDRStyle();
...
*/
  ...
  //// binning setting
  /////////////////////////////////////////////////////////
  double tmpArr1s[9] = {-1.93,-1.20,-0.80,-0.40,0.00,0.40,0.80,1.20,1.93};
  double tmpArr2s[5] = {-1.93,-0.80,0.00,0.80,1.93};
  double tmpArr3s[3] = {-1.93,0.00,1.93};
  ...
  char *Name_Fit[2] = {"Pb-p run", "p-Pb run"};
  ...
  for (int ib =0; ib < nBin; ib ++ ) {
    for(int ifit = 0; ifit < nFit; ifit++){
      //// read files
      if (szAA == "PP" ) { 
        fileIn[ifit][ib]= new TFile(Form("%s/nomfitresults_upsilon_%s_DATA_pt0.0-30.0_y%.2f-%.2f_muPt4.0.root",fileLoc[ifit].Data(),szAA.Data(),binArr[ib],binArr[ib+1]));
      }
      else if (szAA == "PA" ) { 
        fileIn[ifit][ib]= new TFile(Form("%s/nomfitresults_upsilon_%s_DATA_pt0.0-30.0_y%.2f-%.2f_muPt4.0.root",fileLoc[ifit].Data(),szAA.Data(),binArr[ib],binArr[ib+1]));
      }
  ...
  for(int ifit=0; ifit<nFit; ifit++){
    ...
    for (int ib =0; ib < nBin; ib ++ ) {
      sigyield = nSig1s[ifit][ib] + nSig2s[ifit][ib] + nSig3s[ifit][ib];
      sigerr = TMath::Sqrt(nSig1sErr[ifit][ib]*nSig1sErr[ifit][ib]+nSig2sErr[ifit][ib]*nSig2sErr[ifit][ib]+nSig3sErr[ifit][ib]*nSig3sErr[ifit][ib]);
      h1_nSig[ifit]->SetBinContent(ib+1, sigyield); h1_nSig[ifit]->SetBinError(ib+1,sigerr);  
  ...
  gSig[0]->GetYaxis()->SetTitle("dN/dy_{CM}");
  //gSig[0]->GetYaxis()->SetTitle("B #frac{d#sigma}{ dp_{T}} (nb/ GeV/c)");
  gSig[0]->GetXaxis()->SetTitle("y_{CM}");
  gSig[0]->GetXaxis()->SetRangeUser(-1.93,1.93);
  gSig[0]->GetXaxis()->SetLimits(-1.93,1.93);
  ...
  double leg_posy1 = 0.67;
  double leg_posx2 = 0.83;
  double leg_posy2 = 0.77;
...
//  gPad->SetLogy();  // KTO for yield
  ...
  latex->DrawLatex(pos_x,pos_y,Form("%s  #Upsilon(1S+2S+3S)",szAA.Data()));
  ...
  c_nSigs->SaveAs(Form("rap_nSig_%s_%dSbin.pdf",szAA.Data(),states));
...
//  gPad->SetLogy();  // KTO for yield
  ...
  c_nBkg->SaveAs(Form("rap_nBkg%ds_%s_constrain.pdf",states,szAA.Data()));
