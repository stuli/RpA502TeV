...
void draw_RpA_pt_2D(bool isArrow=false)
{
  setTDRStyle();
...
//  double relsys = 0.1;
  ...
  for (int is=0; is<nState; is++){
    ...
    hSys_l[is]=(TH1D*)fInSys[is]->Get("hptRPA_merged1");
    hSys_h[is]=(TH1D*)fInSys[is]->Get("hptRPA_merged2");
...
//  globtex->DrawLatex(0.22, sz_init-sz_step, "p_{T}^{#mu#mu} < 30 GeV/c");
  globtex->DrawLatex(0.73, sz_init-sz_step, "-1.93 < y_{CM}^{#varUpsilon} < 0");
...
//  globtex->DrawLatex(0.22, sz_init, "p_{T}^{#mu} > 4 GeV/c");
//  globtex->DrawLatex(0.22, sz_init-sz_step, "p_{T}^{#mu#mu} < 30 GeV/c");
  globtex->DrawLatex(0.73, sz_init-sz_step, "0 < y_{CM}^{#varUpsilon} < 1.93");
//  globtex->DrawLatex(0.22, sz_init-sz_step*2, "|#eta^{#mu}| < 1.93");
//  globtex->DrawLatex(0.22, sz_init-sz_step*2, "Cent. 0-100%");
