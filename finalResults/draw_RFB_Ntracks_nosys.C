...
void draw_RFB_Ntracks_nosys(bool isArrow=false)
{
  setTDRStyle();
  writeExtraText = true;       // if extra text
  int iPeriod = 3; // 1: pp, 3: pPb, 3: PbPb, 100: RAA vs cent, 101: RAA vs pt or rap
  int iPos = 33;
...
//  double relsys = 0.1;
  ...
  double exsys_1s[5] =  {10, 5, 5,40,140};
  double exsys_2s[5] =  {10,5,5,40,140};
  double exsys_3s[5] =  {10,5,5,40,140};
  ...
  for (int is=0; is<nState; is++){
    ...
    gRPA[is]=(TGraphErrors*)fIn[is]->Get("gRFB");
    gRPA_sys[is]=(TGraphErrors*)fIn[is]->Get("gRFB");
    //cout << "gRPA["<<is<<"] = " <<gRPA[is]->GetPoint(ipt, pxtmp, pytmp) << endl;
  }
  //// read input file : syst.
  TFile* fInSys[nState];
  TH1D* hSys[nState];
  int npoint[nState] = {5,5,5};
  for (int is=0; is<nState; is++){
...
// */
  ...
  //// legend
  //// axis et. al
  gRPA_sys[0]->GetXaxis()->SetTitle("N_{tracks}");
  gRPA_sys[0]->GetXaxis()->CenterTitle();
  gRPA_sys[0]->GetYaxis()->SetTitle("R_{FB} (uncorrected)");
...
//    else {gRPA[is]->Draw("P");}
  ...
  TLegend *leg= new TLegend(0.214, 0.62, 0.289, 0.796);
  SetLegendStyle(leg);
  ...
  arrLeg->SetLineWidth(2);
    ...
    leg -> AddEntry(gRPA[0]," #Upsilon(1S), 1.2 < |y^{#mu#mu}| < 1.93","lp");
    leg -> AddEntry(gRPA[1]," #Upsilon(2S), 0.8 < |y^{#mu#mu}| < 1.93","lp");
    leg -> AddEntry(gRPA[2]," #Upsilon(3S), |y^{#mu#mu}| < 1.93","lp");
...
//  globtex->DrawLatex(0.22, sz_init-sz_step*2, "Cent. 0-100%");
  ...
  CMS_lumi_raaCent( c1, iPeriod, iPos );
	...
	c1->Update();
  c1->SaveAs("RFB_vs_ntracks.pdf");
  c1->SaveAs("RFB_vs_ntracks.png");
