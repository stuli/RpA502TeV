...
void draw_CrossSection_pt_isArrow_run12_nosys_3S(int ppAA=2, bool isArrow=false, int State=1) //1=pp, 2=AA
{
  setTDRStyle();
...
//  double relsys = 0.1;
  ...
  if(State==1){
    ...
    gCrossSection_sys[State-1]->SetMaximum(0.009);
  }
  else if(State==2){
    gCrossSection_sys[State-1]->SetMinimum(0);
    gCrossSection_sys[State-1]->SetMaximum(0.009);
...
//  globtex->DrawLatex(0.27, sz_init-sz_shift-sz_step*2, "|#eta^{#mu}| < 2.4");
  ...
  CMS_lumi( c1, 3, iPos );
	...
	c1->Update();
  c1->SaveAs(Form("Comp_Run12_CrossSection_vs_pt_%s%dS_nosys_3S.pdf",sz_ppAA.Data(),State));
  c1->SaveAs(Form("Comp_Run12_CrossSection_vs_pt_%s%dS_nosys_3S.png",sz_ppAA.Data(),State));
