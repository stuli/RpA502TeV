...
void draw_CrossSection_rap_isArrow_run2(int ppAA=2, bool isArrow=false) //1=pp, 2=AA
{
  setTDRStyle();
...
//  double relsys = 0.1;
  ...
  for (int is=0; is<nState; is++){
  	fIn[is] = new TFile(Form("Cross_Ups_%d_1D_run2.root",is+1),"READ");
...
//  globtex->DrawLatex(0.27, sz_init-sz_shift-sz_step*2, "|#eta^{#mu}| < 2.4");
  ...
  CMS_lumi( c1, 3, iPos );
	...
	c1->Update();
  c1->SaveAs(Form("Run2_CrossSection_vs_rap_%s.pdf",sz_ppAA.Data()));
  c1->SaveAs(Form("Run2_CrossSection_vs_rap_%s.png",sz_ppAA.Data()));
