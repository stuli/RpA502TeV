...
void draw_CrossSection_pt_isArrow_nosys_pp(int ppAA=2, bool isArrow=false) //1=pp, 2=AA
{
  setTDRStyle();
...
//  globtex->DrawLatex(0.27, sz_init-sz_shift-sz_step*2, "|#eta^{#mu}| < 2.4");
  ...
  CMS_lumi( c1, 3, iPos );
	...
	c1->Update();
  c1->SaveAs(Form("PP_CrossSection_vs_pt_%s_nosys.pdf",sz_ppAA.Data()));
  c1->SaveAs(Form("PP_CrossSection_vs_pt_%s_nosys.png",sz_ppAA.Data()));

	///////////////////////////////////////////////////////////////////
	//// save as a root file
	TFile *outFile = new TFile("PP_CrossSection_vs_pt_nosys.root", "RECREATE");
	outFile->cd();
	for (int is=0; is<nState; is++){
