...
void draw_CrossSection_rap_isArrow_run1(int ppAA=2, bool isArrow=false) //1=pp, 2=AA
{
  setTDRStyle();
...
//  globtex->DrawLatex(0.27, sz_init-sz_shift-sz_step*2, "|#eta^{#mu}| < 2.4");
  ...
  CMS_lumi( c1, 3, iPos );
	...
	c1->Update();
  c1->SaveAs(Form("Run1_CrossSection_vs_rap_%s.pdf",sz_ppAA.Data()));
  c1->SaveAs(Form("Run1_CrossSection_vs_rap_%s.png",sz_ppAA.Data()));
