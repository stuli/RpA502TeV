...
//  double relsys = 0.1;
  ...
  for (int is=0; is<nState; is++){
    ...
    gCrossSection[is]=(TGraphErrors*)fIn[is]->Get(Form("gCross%s_int",PPorNot.Data()));
    gCrossSection_sys[is]=(TGraphErrors*)fIn[is]->Get(Form("gCross%s_int",PPorNot.Data()));
  }
  ...
  for (int is=0; is<nState; is++){
    ...
    hSys[is]=(TH1D*)fInSys[is]->Get(Form("hint%s_merged",sz_ppAA.Data()));
    //hSys[is]=(TH1D*)fInSys[is]->Get(Form("hint%s_merged","PA"));
...
//  globtex->DrawLatex(0.27, sz_init-sz_shift-sz_step*2, "|#eta^{#mu}| < 2.4");
  ...
  if (ppAA==2) CMS_lumi( c1, 3, iPos );
	...
	c1->Update();
  c1->SaveAs(Form("plots/CrossSection_vs_int_%s_linear.pdf",sz_ppAA.Data()));
  c1->SaveAs(Form("plots/CrossSection_vs_int_%s_linear.png",sz_ppAA.Data()));

	///////////////////////////////////////////////////////////////////
	//// save as a root file
	TFile *outFile = new TFile(Form("plots/CrossSection%s_vs_int.root",PPorNot.Data()), "RECREATE");
	outFile->cd();
	for (int is=0; is<nState; is++){
  ...
  for (int is=0; is<nState; is++){
    ...
    cout << "$\\pt < 30$ \\GeVc & " << Form("%.4f",val[0])  << " & " << Form("%.4f",val_stat[0]) << " & " << Form("%.5f",val_sys[0]) << " \\\\ " << endl;
  }
  return;
