...
//  globtex->DrawLatex(0.22, sz_init-sz_step*2, "|#eta^{#mu}| < 2.4");
  ...
  g_shade->SetLineWidth(1.);
  g_max->SetLineColor(kGray+3);
  g_min->SetLineColor(kGray+3);
  
  g_shade->SetFillStyle(3004);
  g_shade->SetFillColor(kGray+2);
  ...
  TLegend *leg_th= new TLegend(0.47, 0.59, 0.72, 0.66);
  SetLegendStyle(leg_th);
  leg_th -> SetTextSize(0.034);
  leg_th -> AddEntry(g_shade,"R. Vogt, EPS09 NLO","f");
  leg_th -> Draw("same");
  
  CMS_lumi_raaCent( c1, iPeriod, iPos );

	c1->Update();
  c1->SaveAs(Form("../plots/theory_vogt_%dS_comp_RpA_vs_rap_LHC.pdf",istate));
  c1->SaveAs(Form("../plots/theory_vogt_%dS_comp_RpA_vs_rap_LHC.png",istate));
