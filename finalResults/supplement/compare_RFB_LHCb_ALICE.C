...
void compare_RFB_LHCb_ALICE(int istate=1) //1 or 2 (1S or 2S)
{
  TH1::SetDefaultSumw2();
  setTDRStyle();
  ...
  const int nfile = 3; // 0: ALICE, 1: ATLAS, 2: LHCb, 3: CMS
  double xmin = 0;
  double xmax = 4.3;
//  double relsys = 0.1;
  ...
  //// ALICE values
  const int cn_alice=  1;
  double cpx_alice[cn_alice] =  {3.245};
  double cpy_alice[cn_alice] =  {0.95};
  double cex_alice[cn_alice] =  {0.};
  double cey_alice[cn_alice] =  {0.24};
  double cexsys_alice[cn_alice] =  {0.57/2};
  double ceysys_alice[cn_alice] = {0.14};
  ...
  TFile *f_acc = new TFile(Form("../../Acceptance/20180724/acceptance_wgt_%dS_20180724_2Dplot.root",istate),"read");
  TFile *f_eff = new TFile(Form("../../Efficiency/RootFiles/EffNomCor_SysRFB_%dS.root",istate),"read");
  ...
  double corr_acc1 = hAcc_r->GetBinContent(1);
  double corr_acc2 = hAcc_r->GetBinContent(2);
  double corr_acc = corr_acc1/corr_acc2;
  ...
  // eff 
  TFile* feff = new TFile("../../Efficiency/RootFiles/EffNomCor_SysRFB_1S.root");
  TH1D* heff = (TH1D*)feff->Get("EffSysIntRFB");
  // acceptance
  TFile* facc = new TFile("../../Acceptance/20180724/sys_acceptance_ups1S_20180724.root");
  ...
  //// 2) ours
  gRAA[2]= new TGraphErrors(hRFB1);
  gRAA_sys[2] = new TGraphErrors(hRFB1);

  //// set bin width and calculate systematic uncertainties 
  double pxtmp, pytmp, extmp, eytmp;
  double relsys;
  int npoint = 1; 
  if (npoint != gRAA[2]->GetN()) {cout << "Error!! data file and syst. file have different binnig!" << endl; return; }
  for (int ipt=0; ipt< npoint; ipt++) {
    pxtmp=0; pytmp=0; extmp=0; eytmp=0; relsys=0;
    gRAA[2]->GetPoint(ipt, pxtmp, pytmp);
    cout << "pxtmp : " << pxtmp << endl;
    extmp=gRAA[2]->GetErrorX(ipt);
    eytmp=gRAA[2]->GetErrorY(ipt);
    relsys=sys_tot;
    // 1) remove ex from gRAA
    gRAA[2]->SetPointError(ipt, 0, eytmp);
    // 2) set ey for gRAA_sys (assign 10% temporarily)
    //gRAA_sys[1]->SetPointError(ipt, extmp, pytmp*relsys);
    gRAA_sys[2]->SetPointError(ipt, exsys_1s[ipt],pytmp*relsys);
  }
  ////////////////////////////////////////////////////////////////
  
  //// graph style 
  SetGraphStyle_comp2(gRAA[0], 1, 1); 
  SetGraphStyleSys_comp2(gRAA_sys[0], 1); 
  SetGraphStyle_comp2(gRAA[1], 2, 2); 
  SetGraphStyleSys_comp2(gRAA_sys[1], 2); 
  SetGraphStyle(gRAA[2], 0, 0); 
  SetGraphStyleSys(gRAA_sys[2], 0); 
  ...
  leg -> AddEntry(gRAA[2],Form("#varUpsilon(%dS), p_{T}^{#varUpsilon} < 30 GeV/c",istate),"lp");
...
//  globtex->DrawLatex(0.22, sz_init-sz_step*2, "|#eta^{#mu}| < 2.4");
  ...
  CMS_lumi_raaCent( c1, iPeriod, iPos );
	...
	c1->Update();
  c1->SaveAs(Form("../plots/%dS_comp_RFB_vs_rap_LHCb_ALICE.pdf",istate));
  c1->SaveAs(Form("../plots/%dS_comp_RFB_vs_rap_LHCb_ALICE.png",istate));
