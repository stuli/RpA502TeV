...
void draw_CrossSection_rap_isArrow_run12_nosys_3S(int ppAA=2, bool isArrow=false, int State=1) //1=pp, 2=AA
{
  setTDRStyle();
...
//  double relsys = 0.1;
  ...
  for (int is=0; is<nState; is++){
  	fIn[is] = new TFile(Form("Cross_Ups_%d_1D_run1_3S.root",is+1),"READ");
    gCrossSection[is]=(TGraphErrors*)fIn[is]->Get("gCross_rap");
    gCrossSection_sys[is]=(TGraphErrors*)fIn[is]->Get("gCross_rap");
  	fIn_1[is] = new TFile(Form("Cross_Ups_%d_1D_run2_3S.root",is+1),"READ");
    gCrossSection_1[is]=(TGraphErrors*)fIn_1[is]->Get("gCross_rap");
    gCrossSection_sys_1[is]=(TGraphErrors*)fIn_1[is]->Get("gCross_rap");
  ...
  //// axis et. al
  gCrossSection_sys[State-1]->GetXaxis()->SetTitle("y^{#varUpsilon}_{CM}");
  ...
  if(State==1){
    ...
    gCrossSection_sys[State-1]->SetMaximum(0.07);
  }
  else if(State==2){
    gCrossSection_sys[State-1]->SetMinimum(0);
    gCrossSection_sys[State-1]->SetMaximum(0.07);
...
//  gPad->SetLogy(1); // for cross section
  ...
  TLegend *leg= new TLegend(0.62, 0.55, 0.83, 0.72);
  SetLegendStyle(leg);
  ...
  leg -> AddEntry(gCrossSection[State-1]," #Upsilon(3S) Run1","lp");
  leg -> AddEntry(gCrossSection_1[State-1]," #Upsilon(3S) Run2","lp");
  leg->Draw("same");
...
//  globtex->DrawLatex(0.27, sz_init-sz_shift-sz_step*2, "|#eta^{#mu}| < 2.4");
  ...
  CMS_lumi( c1, 3, iPos );
	...
	c1->Update();
  c1->SaveAs(Form("Comp_Run12_CrossSection_vs_rap_%s%dS_nosys_3S.pdf",sz_ppAA.Data(),State));
  c1->SaveAs(Form("Comp_Run12_CrossSection_vs_rap_%s%dS_nosys_3S.png",sz_ppAA.Data(),State));
