...
void doFitUpsilon_Data_AllParmFree_pa_yCM_ne0800(
       int collId = kPADATA,  
       float ptLow=0, float ptHigh=30, 
       float yLow=-0.33, float yHigh=0.47,
       ...
       )
{
  ...
  float eta_low = -2.4;
  float eta_high = 2.4;
  TString kineLabel = getKineLabel (collId, ptLow, ptHigh, yLow-0.47, TMath::Abs(yHigh-0.47), muPtCut, cLow, cHigh, dphiEp2Low, dphiEp2High) ;
  ...
  RooRealVar alpha1s_1("alpha1s_1","tail shift", 2.071 , 1.613, 3.322);
  ...
  RooRealVar n1s_1("n1s_1","power order", 2.235 , 1.510, 3.237);
...
//  if(yLow==0) drawText(Form("|y^{#mu#mu}| < %.1f",yHigh ), pos_text_x,pos_text_y-pos_y_diff,text_color,text_size);
  drawText(Form("%.2f < y^{#mu#mu}_{CM} < %.2f",yLow-0.47,TMath::Abs(yHigh-0.47) ), pos_text_x,pos_text_y-pos_y_diff,text_color,text_size);
  if(collId != kPPDATA && collId != kPPMCUps1S && collId != kPPMCUps2S) 
  {
    ...
    drawText(Form("p_{T}^{#mu} > %.f GeV/c", muPtCut ), pos_text_x,pos_text_y-pos_y_diff*2,text_color,text_size);
    drawText(Form("|#eta^{#mu}_{CM}| < ",eta_high), pos_text_x,pos_text_y-pos_y_diff*3,text_color,text_size);
  }  
