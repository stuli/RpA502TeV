       ...
       bool fixParameters=0  )
{
  ...
  TString kineCut = Form("pt>%.2f && pt<%.2f && abs(y)>%.2f && abs(y)<%.2f && abs(eta1)<%.2f && abs(eta1)>%.2f && abs(eta2)<%.2f && abs(eta2)>%.2f ",ptLow, ptHigh, yLow, yHigh, eta_high,eta_low, eta_high,eta_low);
