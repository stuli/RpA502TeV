...
void doFitUpsilon_Data_AllParmFree_pp_yCM_12193(
       int collId = kPPDATA,  
       ...
       )
{
  ...
  TString kineCut = Form("pt>%.2f && pt<%.2f && TMath::Abs(y)>%.2f && TMath::Abs(y)<%.2f && eta1<%.2f && eta1>%.2f && eta2<%.2f && eta2>%.2f",ptLow, ptHigh, yLow, yHigh, eta_high,eta_low, eta_high,eta_low );
  ...
  RooRealVar alpha1s_1("alpha1s_1","tail shift", 2.871 , 1.993, 3.312);
  ...
  RooRealVar n1s_1("n1s_1","power order", 2.235 , 1.953, 3.017);
...
//  if(yLow==0) drawText(Form("|y^{#mu#mu}| < %.1f",yHigh ), pos_text_x,pos_text_y-pos_y_diff,text_color,text_size);
  drawText(Form("%.2f < |y^{#mu#mu}_{CM}| < %.2f",yLow,TMath::Abs(yHigh) ), pos_text_x,pos_text_y-pos_y_diff,text_color,text_size);
