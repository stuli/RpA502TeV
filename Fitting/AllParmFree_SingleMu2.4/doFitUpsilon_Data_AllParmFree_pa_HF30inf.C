...
void doFitUpsilon_Data_AllParmFree_pa_HF30inf(
       int collId = kPADATA,  
       ...
       )
{
  ...
  TString kineCut = Form("pt>%.2f && pt<%.2f && y>%.2f && y<%.2f && eta1<%.2f && eta1>%.2f && eta2<%.2f && eta2>%.2f && (hfpluseta4+hfminuseta4)>%d",ptLow, ptHigh, yLow, yHigh, eta_high,eta_low, eta_high,eta_low, hflow);
  ...
  RooRealVar alpha1s_1("alpha1s_1","tail shift", 2.071 , 1.513, 3.302);
...
//  if(yLow==0) drawText(Form("|y^{#mu#mu}| < %.1f",yHigh ), pos_text_x,pos_text_y-pos_y_diff,text_color,text_size);
  ...
  TFile* outf = new TFile(Form("AllParmFree_fitresults_upsilon_DoubleCB_5TeV_%s_hf%d-%d.root",kineLabel.Data(),hflow),"recreate");
  outh->Write();
  c1->SaveAs(Form("AllParmFree_fitresults_upsilon_DoubleCB_5TeV_%s_hf%dinf.pdf",kineLabel.Data(),hflow));
  c1->SaveAs(Form("AllParmFree_fitresults_upsilon_DoubleCB_5TeV_%s_hf%dinf.png",kineLabel.Data(),hflow));
  c1->Write();
  ws->Write();
