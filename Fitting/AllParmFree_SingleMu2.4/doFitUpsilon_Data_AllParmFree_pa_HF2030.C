...
void doFitUpsilon_Data_AllParmFree_pa_HF2030(
       int collId = kPADATA,  
       float ptLow=0, float ptHigh=30, 
       float yLow=-1.46, float yHigh=2.4,
       int cLow=0, int cHigh=200,
       int hflow=20, int hfhigh=30,
       float muPtCut=4.0
       )
{
  float dphiEp2Low = 0 ;
  ...
  if ( collId == kPADATA) 
  {
    f1 = new TFile("/home/deathold/work/CMS/analysis/Upsilon_RpA/UpsilonpPb5TeV/RpA5.02TeV/skimmedFiles/yskimPA1st_OpSign_20177262037_unIdentified.root");
  ...
  TString kineCut = Form("pt>%.2f && pt<%.2f && y>%.2f && y<%.2f && eta1<%.2f && eta1>%.2f && eta2<%.2f && eta2>%.2f && (hfpluseta4+hfminuseta4)>%d && (hfpluseta4+hfminuseta4)<%d ",ptLow, ptHigh, yLow, yHigh, eta_high,eta_low, eta_high,eta_low, hflow, hfhigh );
  ...
  RooRealVar alpha1s_1("alpha1s_1","tail shift", 2.071 , 1.413, 3.302);
  ...
  RooRealVar n1s_1("n1s_1","power order", 2.265 , 1.506, 3.357);
...
//  if(yLow==0) drawText(Form("|y^{#mu#mu}| < %.1f",yHigh ), pos_text_x,pos_text_y-pos_y_diff,text_color,text_size);
  ...
  TFile* outf = new TFile(Form("AllParmFree_fitresults_upsilon_DoubleCB_5TeV_%s_hf%d-%d.root",kineLabel.Data(),hflow,hfhigh),"recreate");
  outh->Write();
  c1->SaveAs(Form("AllParmFree_fitresults_upsilon_DoubleCB_5TeV_%s_hf%d-%d.pdf",kineLabel.Data(),hflow,hfhigh));
  c1->SaveAs(Form("AllParmFree_fitresults_upsilon_DoubleCB_5TeV_%s_hf%d-%d.png",kineLabel.Data(),hflow,hfhigh));
  c1->Write();
  ws->Write();
