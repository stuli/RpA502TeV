...
void Raa_3S_Workspace(
  ...
  const string name_pbpb=Form("/afs/cern.ch/work/g/goni/Analysis/Upsilon/upslion_Raa/Usercode/upperlimit/fitResults/fitresults_upsilon_DoubleCB_AA_DATA_pt%.1f-%.1f_y%.1f-%.1f_muPt%.1f_centrality%d-%d_dphiEp_0.00PI_100.00PI.root",ptLow,ptHigh,yLow,yHigh,muPtCut,cLow,cHigh);
  const string name_pp=Form("/afs/cern.ch/work/g/goni/Analysis/Upsilon/upslion_Raa/Usercode/upperlimit/fitResults/fitresults_upsilon_DoubleCB_PP_DATA_pt%.1f-%.1f_y%.1f-%.1f_muPt%.1f.root",ptLow,ptHigh,yLow,yHigh,muPtCut);

  const string name_out=Form("/afs/cern.ch/work/g/goni/Analysis/Upsilon/upslion_Raa/Usercode/upperlimit/results/%s/fitResults_combo_pt%.1f_%.1f_y%.1f_%.1f_cent%d_%d_muPt%.1f.root",poiname.c_str(),ptLow,ptHigh,yLow,yHigh,cLow,cHigh,muPtCut);
  ...
  RooRealVar* raa3 = new RooRealVar("raa3","Ratio(#Upsilon(3S)_{PbPb}/#Upsilon(3S)_{pp})",.5,-1,1);
  ...
  ws->import(step);
  ws->factory( "Uniform::flat(raa3)" );//ws->factory( "Uniform::flat(dubRat)" );

  RooRealVar* nsig1_hi = ws->var("nSig1s_hi");//RooRealVar* nsig1_hi = ws->var("N_{#varUpsilon(1S)}_hi");
  RooRealVar* nsig2_hi = ws->var("nSig2s_hi");
  //RooRealVar* nsig3_1_hi = ws->var("nSig3s_hi");
  RooRealVar* nsig3_pp = ws->var("nSig3s_pp");
  cout << "!!!!!!!# of N#Upsilon3S_pp!!!!!!! : " << nsig3_pp << endl;
  RooFormulaVar nsig3_hi_modified("nsig3_hi_modified", "@0*@1", RooArgList(*raa3, *nsig3_pp));
  ...
  ws->factory( "SUM::nbkg_hi_nom(alpha_nbkg_hi*bkgLowPt_hi)" );//ws->factory( "SUM::nbkg_hi_nom(alpha_nbkg_hi*bkgPdf_hi)" );
  ...
  ws->factory( "SUM::nbkg_pp_nom(alpha_nbkg_pp*bkgLowPt_pp)" );//ws->factory( "SUM::nbkg_pp_nom(alpha_nbkg_pp*bkgPdf_pp)" );
  ...
  RooArgSet poi("poi");
  poi.add( *ws->var("raa3") );//poi.add( *ws->var("dubRat") );
  ...
  ws->var("glob_nbkg_hi")->setConstant(true);
  ws->var("glob_nbkg_pp")->setConstant(true);
  ws->var("mass")->setConstant(true);//ws->var("invariantMass")->setConstant(true);
  ...
  RooPlot *framepoi = ((RooRealVar *)poi.first())->frame(Bins(10),Range(0.,.3),Title("LL and profileLL in Yield(#Upsilon(3S))"));
  ...
  //cpoi->SaveAs(Form("results/%s/cpoi_%s_%s_%s.pdf",poi.Data(),ptbin.Data(),ybin.Data(),centbin.Data()));
  cpoi->SaveAs(Form("results/raa3/cpoi_pt%.2f-%.2f_y%.2f-%.2f_muPt%.2f_centrality%d-%d.pdf",ptLow,ptHigh,yLow,yHigh,muPtCut,cLow,cHigh));
  ...
  c1->cd(); xframeSB->Draw();
  c1->SaveAs(Form("results/raa3/c1_pt%.2f-%.2f_y%.2f-%.2f_muPt%.2f_centrality%d-%d.pdf",ptLow,ptHigh,yLow,yHigh,muPtCut,cLow,cHigh));
  ...
  c2->cd(); xframeB->Draw();
  c2->SaveAs(Form("results/raa3/c2_pt%.2f-%.2f_y%.2f-%.2f_muPt%.2f_centrality%d-%d.pdf",ptLow,ptHigh,yLow,yHigh,muPtCut,cLow,cHigh));
