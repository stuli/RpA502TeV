...
void dimuEff_oniaMode3_pPb_HFeta4_5(
...
cout << ptBinEdges << endl;
	...
	TCanvas *preCan1 = new TCanvas();
	SumET_HF_Weights->Draw();
	preCan1->SaveAs(Form("eff_pPb/HFWeights_%dS_%s_HFeta4_5.png",oniaMode,ispPb ? "pPb" : "PP"));
	TH1D  *Ntracks_Weights = new TH1D("Ntracks_Weights", "Ntracks_Weights", ispPb ? nNtracksBin : 1, ispPb ? NtracksBinEdges : IntBinEdges);
	TCanvas *preCan2 = new TCanvas();
	Ntracks_Weights->Draw();
	preCan2->SaveAs(Form("eff_pPb/NtracksWeights_%dS_%s_HFeta4_5.png",oniaMode,ispPb ? "pPb" : "PP"));
	//cout<<"STILL WORKING"<<endl;
	...
	Ntracks_Weights->SetTitle("N_{tracks} Weights");
...
// */
			...
			if (rapLow < rapReco < rapHigh && ptReco < 30 && Centrality < 200&&TMath::Abs(qq4mom->Eta())<5&&TMath::Abs(qq4mom->Eta())>4){
...
//			rapGen = TMath::Abs(g_qq4mom->Rapidity());
			...
			if (rapLow < rapGen < rapHigh && ptGen < 30 && Centrality < 200 &&TMath::Abs(g_qq4mom->Eta())<5&&TMath::Abs(g_qq4mom->Eta())>4){
...
c1->SaveAs(Form("eff_pPb/EfficiencyCent_%dS_%s_HFeta4_5.png",oniaMode,ispPb ? "pPb" : "PP"));
}
...
c2->SaveAs(Form("eff_pPb/EfficiencyPt_%dS_%s_HFeta4_5.png",oniaMode,ispPb ? "pPb" : "PP"));
...
c3->SaveAs(Form("eff_pPb/EfficiencyRap_%dS_%s_HFeta4_5.png",oniaMode,ispPb ? "pPb" : "PP"));
...
c4->SaveAs(Form("eff_pPb/EfficiencyInt_%dS_%s_HFeta4_5.png",oniaMode,ispPb ? "pPb" : "PP"));
...
c5->SaveAs(Form("eff_pPb/EfficiencyNtracks_%dS_%s_HFeta4_5.png",oniaMode,ispPb ? "pPb" : "PP"));
}
...
c6->SaveAs(Form("eff_pPb/EfficiencySumET_HF_%dS_%s_HFeta4_5.png",oniaMode,ispPb ? "pPb" : "PP"));
}

TFile* MyFileEff;
MyFileEff = new TFile(Form("eff_pPb/Eff_%s_%dS_HFeta4_5.root",ispPb ? "pPb" : "PP",oniaMode), "Recreate");
if(ispPb){
	EffSumET_HF->Write();
	EffNtracks->Write();
	EffCent->Write();
