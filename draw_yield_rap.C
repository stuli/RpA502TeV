...
int draw_yield_rap(TString szAA = "PP", int states =1)
{
  ...
  double tmpArr1s[11] = {-1.93,-1.6,-1.2,-0.8,-0.4,0, 0.4, 0.8, 1.2, 1.6, 1.93};
  double tmpArr2s[4] = {0.0, 0.8, 1.6, 2.4};
  double tmpArr3s[3] = {0.0, 1.2, 2.4};

  int tmpBin;
  if ( states ==1) {
    cout << " ***** 1S *****" << endl; tmpBin = 10;
  }else if (states ==2){
  ...
  for (int ib =0; ib < nBin; ib ++ ) {
    ...
    else if (szAA == "PA" ) { fileIn[ib]= new TFile(Form("/home/deathold/work/CMS/analysis/Upsilon_RpA/UpsilonpPb5TeV/Fitting/AllParmFree/AllParmFree_fitresults_upsilon_DoubleCB_5TeV_%s_DATA_pt0.0-30.0_y%.2f-%.2f_muPt4.0.root",szAA.Data(),binArr[ib],binArr[ib+1])); }
